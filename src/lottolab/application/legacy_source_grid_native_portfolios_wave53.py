"""Frozen-runtime source-grid portfolio reproduction for BIG_LOTTO wave 53."""
...
SOURCE_NATIVE_WAVE53_PROTOCOL = "legacy_source_grid_native_wave53/v1"
DEFAULT_SOURCE_NATIVE_WAVE53_USER_SEED = (
    "biglotto-full-universe-source-grid-native-wave53-v1"
)
...
LEDGER_RESOURCE_NAME = "biglotto_source_grid_wave53_ticket_ledger_v1.json"
LEDGER_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_WAVE53_TICKET_LEDGER_V1"
LEDGER_FILE_SHA256 = "519b25c61e374cacce762d0ce4aaceb2813500260867c8443fec03418182cefd"
LEDGER_CONTENT_SHA256 = "201233318fe457d53f8f614f1fc4e3cc165b004493c8f0c75f58eac2ceddc675"
...
SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: (
...
MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: 500,
        RGF_METHOD_ID: 200,
    }
)
MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: "SOURCE_FOURIER_RHYTHM_DECLARED_WINDOW_500",
        RGF_METHOD_ID: "SOURCE_GMM_BURN_IN_200_WALK_FORWARD_BOUNDARY",
    }
)
NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: 14,
        RGF_METHOD_ID: 6,
    }
)
SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: (
...
SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        method_id: len(members)
        for method_id, members in (
            SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE53_METHOD.items()
        )
    }
)
SOURCE_CANDIDATE_K_VALUES_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: (49,),
        RGF_METHOD_ID: (49,),
    }
)
NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        method_id: (
            f"{NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]}_"
            "SOURCE_POSITIONAL_TICKETS_ACROSS_DECLARED_CONFIGURATION_ORDER"
        )
        for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS
    }
)
INTRA_TICKET_ORDER_SEMANTICS_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: (
            "SOURCE_LIST_VALUES_CANONICALIZED_ASCENDING_WITHOUT_POSITIONAL_REORDER"
        ),
        RGF_METHOD_ID: (
            "SOURCE_SET_TOP6_CANONICALIZED_ASCENDING_WITHOUT_CONFIGURATION_REORDER"
        ),
    }
)
RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: "DETERMINISTIC_NO_RANDOM_SELECTION",
        RGF_METHOD_ID: "DETERMINISTIC_NO_RANDOM_SELECTION",
    }
)
RANDOMNESS_USED_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: False,
        RGF_METHOD_ID: False,
    }
)
SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: "OLDEST_FIRST",
        RGF_METHOD_ID: "OLDEST_FIRST",
    }
)
SOURCE_HISTORY_ORDER_DETAIL_BY_SOURCE_NATIVE_WAVE53_METHOD: Final = MappingProxyType(
    {
        ANALYSIS_METHOD_ID: (
            "SOURCE_DATE_AND_DRAW_ASC_THEN_STRICT_PREFIX_BEFORE_TARGET"
        ),
        RGF_METHOD_ID: (
            "SOURCE_WALK_FORWARD_DRAWS_OLDEST_FIRST_THEN_HISTORY_SLICE_"
            "STRICTLY_BEFORE_TARGET"
        ),
    }
)


class LegacySourceGridNativeWave53Error(ValueError):
    """The request or packaged source-runtime ledger is invalid."""


class LegacySourceGridNativeWave53SourceError(LegacySourceGridNativeWave53Error):
    """A target cannot produce a source-valid wave-53 portfolio."""
...
class LegacySourceGridNativeWave53Request:
    legacy_method_id: str
    target_draw_number: str
    history: tuple[LegacyHistoryDraw, ...]
    dataset_sha256: str
    replicate_id: int = 0
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE53_USER_SEED


@dataclass(frozen=True, slots=True)
class LegacySourceGridNativeWave53Metadata:
    protocol: str
    legacy_method_id: str
    source_sha256: str
    target_draw_number: str
...
@dataclass(frozen=True, slots=True)
class LegacySourceGridNativeWave53Result:
    tickets: tuple[Ticket, ...]
    metadata: LegacySourceGridNativeWave53Metadata
...
def _ticket(value: object) -> Ticket:
    if not isinstance(value, list):
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ticket must be an array"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ticket is not a sorted legal ticket"
        )
...
def _load_ledger() -> _Ledger:
    ...
    if hashlib.sha256(raw).hexdigest() != LEDGER_FILE_SHA256:
        raise LegacySourceGridNativeWave53Error("packaged wave-53 ledger file SHA changed")
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ledger is invalid JSON"
        ) from exc
    if not isinstance(parsed, dict):
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ledger must be an object"
        )
    ...
    if (
        ...
        != dict(MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE53_METHOD)
        or document.get("minimum_history_rationale_by_method")
        != dict(MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE53_METHOD)
        or document.get("source_sha256_by_method")
        != dict(SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE53_METHOD)
        or document.get("source_configuration_count_by_method")
        != dict(SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD)
        or document.get("source_configuration_members_by_method") != expected_members
    ):
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ledger identity changed"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 ledger layout changed"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave53Error(
            "packaged wave-53 target sequence changed"
        )
    typed_tickets = cast(dict[str, object], tickets_raw)
    if set(typed_tickets) != set(SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS):
        raise LegacySourceGridNativeWave53Error("packaged wave-53 method set changed")
    by_method: dict[str, tuple[tuple[Ticket, ...] | None, ...]] = {}
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS:
        raw_sequence = typed_tickets[method_id]
        if not isinstance(raw_sequence, list):
            raise LegacySourceGridNativeWave53Error(
                "packaged wave-53 method sequence changed"
            )
        ...
        for candidate in cast(list[object], raw_sequence):
            ...
            if not isinstance(candidate, list):
                raise LegacySourceGridNativeWave53Error(
                    "packaged wave-53 portfolio changed"
                )
            portfolio = tuple(_ticket(ticket) for ticket in cast(list[object], candidate))
            if (
                len(portfolio)
                != NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ):
                raise LegacySourceGridNativeWave53Error(
                    "packaged wave-53 native ticket count changed"
                )
            sequence.append(portfolio)
        if len(sequence) != 2148:
            raise LegacySourceGridNativeWave53Error(
                "packaged wave-53 target alignment changed"
            )
...
def load_legacy_source_grid_native_wave53_ledger_for_verification() -> _Ledger:
    """Expose the immutable checksummed ledger to contract tests."""

    return _load_ledger()


def _validate_request(request: LegacySourceGridNativeWave53Request) -> None:
    if request.legacy_method_id not in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS:
        raise LegacySourceGridNativeWave53Error(
            "legacy method is outside the executable wave-53 batch"
        )
    if (
    ...
    minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE53_METHOD[
        request.legacy_method_id
    ]
    if len(request.history) < minimum:
        raise LegacySourceGridNativeWave53SourceError(
...
def generate_legacy_source_grid_native_wave53_portfolio(
    request: LegacySourceGridNativeWave53Request,
) -> LegacySourceGridNativeWave53Result:
    ...
    randomness_used = RANDOMNESS_USED_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
    return LegacySourceGridNativeWave53Result(
        tickets=tickets,
        metadata=LegacySourceGridNativeWave53Metadata(
            protocol=SOURCE_NATIVE_WAVE53_PROTOCOL,
            legacy_method_id=method_id,
            source_sha256=SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id],
            ...
            random_protocol=RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id],
            ...
            source_history_order=(
                SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            source_history_order_detail=(
                SOURCE_HISTORY_ORDER_DETAIL_BY_SOURCE_NATIVE_WAVE53_METHOD[
                    method_id
                ]
            ),
            ...
            source_minimum_history_rationale=(
                MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            context_draw_count=len(request.history),
            context_numbers_sha256=context_sha256,
            candidate_k=None,
            source_candidate_k_values=(
                SOURCE_CANDIDATE_K_VALUES_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            native_ticket_count=len(tickets),
            native_ticket_count_semantics=(
                NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            native_ticket_order=(
            ...
            source_method_combination_count=(
                SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            combination_members=(
                SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
            intra_ticket_order_semantics=(
                INTRA_TICKET_ORDER_SEMANTICS_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
            ),
...
__all__ = [
    ...
    "MODEL_CANDIDATE_K",
    "NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD",
    "NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE53_METHOD",
    "PINNED_DATASET_SHA256",
    "RANDOMNESS_USED_BY_SOURCE_NATIVE_WAVE53_METHOD",
    ...
    "SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE53_METHOD",
    "SOURCE_HISTORY_ORDER_DETAIL_BY_SOURCE_NATIVE_WAVE53_METHOD",
    "SOURCE_NATIVE_WAVE53_PROTOCOL",
    "SOURCE_REFERENCE_RUNTIME",
    "SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE53_METHOD",
    "SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS",
    "LegacySourceGridNativeWave53Error",
    "LegacySourceGridNativeWave53Metadata",
    ...
    "load_legacy_source_grid_native_wave53_ledger_for_verification",
]
