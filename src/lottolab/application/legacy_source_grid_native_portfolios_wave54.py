"""Frozen-runtime source-grid portfolio reproduction for BIG_LOTTO wave 54."""
...
SOURCE_NATIVE_WAVE54_PROTOCOL = "legacy_source_grid_native_wave54/v1"
DEFAULT_SOURCE_NATIVE_WAVE54_USER_SEED = (
    "biglotto-full-universe-source-grid-native-wave54-v1"
)
FROZEN_SOURCE_COMMIT = "49a25effa62fc24f40789c16be6f11bdfb41a4a9"
PINNED_DATASET_SHA256 = "2f3d711cb97cddabfc6d351b5a639614da60dc3473cc23368711f605e3fe2d6b"
SOURCE_REFERENCE_RUNTIME = (
    "CPYTHON_3_9_6_NUMPY_1_26_2_SCIPY_1_12_0_FROZEN_MODULES_"
    "PYTHON_RANDOM_RESET_SEED42_PER_TARGET"
)
LEDGER_RESOURCE_NAME = "biglotto_source_grid_wave54_ticket_ledger_v1.json"
LEDGER_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_WAVE54_TICKET_LEDGER_V1"
LEDGER_FILE_SHA256 = "ae8846b500c6d0ec6754ee992018793605a5716a050d2600c486a3da4d0346b9"
LEDGER_CONTENT_SHA256 = "f40fd33499cc9f43ff53f352505a175ecd35c4cbee7b4a866017eea25c41103a"
...
SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (
...
MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: 50,
        EVOLUTIONARY_GUM_METHOD_ID: 150,
    }
)
MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: "SOURCE_CLI_DEFAULT_WINDOW_50_FULL_CONTEXT_BOUNDARY",
        EVOLUTIONARY_GUM_METHOD_ID: (
            "SOURCE_STABLE_RECIPE_WINDOW_150_FULL_CONTEXT_BOUNDARY"
        ),
    }
)
NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: 2,
        EVOLUTIONARY_GUM_METHOD_ID: 2,
    }
)
SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (
...
SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        method_id: len(members)
        for method_id, members in (
            SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE54_METHOD.items()
        )
    }
)
SOURCE_CANDIDATE_K_VALUES_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (49,),
        EVOLUTIONARY_GUM_METHOD_ID: (49,),
    }
)
NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        method_id: (
            f"{NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]}_"
            "SOURCE_POSITIONAL_TICKETS_ACROSS_DECLARED_CONFIGURATION_ORDER"
        )
        for method_id in SUPPORTED_SOURCE_NATIVE_WAVE54_METHODS
    }
)
INTRA_TICKET_ORDER_SEMANTICS_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (
...
RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (
            "FROZEN_BEST_CONFIG_PYTHON_MODULE_GLOBAL_RANDOM_RESET_SEED42_PER_TARGET"
        ),
        EVOLUTIONARY_GUM_METHOD_ID: (
            "FROZEN_FRONTIER_OR_STABLE_RECIPE_PYTHON_MODULE_GLOBAL_RANDOM_"
            "RESET_SEED42_PER_TARGET"
        ),
    }
)
RANDOMNESS_USED_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: True,
        EVOLUTIONARY_GUM_METHOD_ID: True,
    }
)
SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: "OLDEST_FIRST",
        EVOLUTIONARY_GUM_METHOD_ID: "OLDEST_FIRST",
    }
)
SOURCE_HISTORY_ORDER_DETAIL_BY_SOURCE_NATIVE_WAVE54_METHOD: Final = MappingProxyType(
    {
        CONSENSUS_METHOD_ID: (
            "SOURCE_STRATEGY_LEADERBOARD_NORMALIZED_OLDEST_FIRST_THEN_"
...
class LegacySourceGridNativeWave54Error(ValueError):
    """The request or packaged source-runtime ledger is invalid."""


class LegacySourceGridNativeWave54SourceError(LegacySourceGridNativeWave54Error):
    """A target cannot produce a source-valid wave-54 portfolio."""
...
class LegacySourceGridNativeWave54Request:
...
@dataclass(frozen=True, slots=True)
class LegacySourceGridNativeWave54Metadata:
    protocol: str
    legacy_method_id: str
    source_sha256: str
    target_draw_number: str
...
@dataclass(frozen=True, slots=True)
class LegacySourceGridNativeWave54Result:
    tickets: tuple[Ticket, ...]
    metadata: LegacySourceGridNativeWave54Metadata
...
def _ticket(value: object) -> Ticket:
    if not isinstance(value, list):
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ticket must be an array"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ticket is not a sorted legal ticket"
        )
...
def _load_ledger() -> _Ledger:
    ...
    if hashlib.sha256(raw).hexdigest() != LEDGER_FILE_SHA256:
        raise LegacySourceGridNativeWave54Error("packaged wave-54 ledger file SHA changed")
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ledger is invalid JSON"
        ) from exc
    if not isinstance(parsed, dict):
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ledger must be an object"
        )
    ...
    if (
        ...
        != dict(MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE54_METHOD)
        or document.get("minimum_history_rationale_by_method")
        != dict(MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE54_METHOD)
        or document.get("source_sha256_by_method")
        != dict(SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE54_METHOD)
        or document.get("source_configuration_count_by_method")
        != dict(SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE54_METHOD)
        or document.get("source_configuration_members_by_method") != expected_members
    ):
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ledger identity changed"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 ledger layout changed"
        )
    ...
    if (
        ...
        raise LegacySourceGridNativeWave54Error(
            "packaged wave-54 target sequence changed"
        )
    typed_tickets = cast(dict[str, object], tickets_raw)
    if set(typed_tickets) != set(SUPPORTED_SOURCE_NATIVE_WAVE54_METHODS):
        raise LegacySourceGridNativeWave54Error("packaged wave-54 method set changed")
    by_method: dict[str, tuple[tuple[Ticket, ...] | None, ...]] = {}
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE54_METHODS:
        raw_sequence = typed_tickets[method_id]
        if not isinstance(raw_sequence, list):
            raise LegacySourceGridNativeWave54Error(
                "packaged wave-54 method sequence changed"
            )
        ...
        for candidate in cast(list[object], raw_sequence):
            ...
            if not isinstance(candidate, list):
                raise LegacySourceGridNativeWave54Error(
                    "packaged wave-54 portfolio changed"
                )
            ...
            if (
                ...
                raise LegacySourceGridNativeWave54Error(
                    "packaged wave-54 native ticket count changed"
                )
            sequence.append(portfolio)
        if len(sequence) != 2148:
            raise LegacySourceGridNativeWave54Error(
                "packaged wave-54 target alignment changed"
            )
...
def load_legacy_source_grid_native_wave54_ledger_for_verification() -> _Ledger:
    """Expose the immutable checksummed ledger to contract tests."""

    return _load_ledger()


def _validate_request(request: LegacySourceGridNativeWave54Request) -> None:
    if request.legacy_method_id not in SUPPORTED_SOURCE_NATIVE_WAVE54_METHODS:
        raise LegacySourceGridNativeWave54Error(
            "legacy method is outside the executable wave-54 batch"
        )
    if (
    ...
    minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE54_METHOD[
        request.legacy_method_id
    ]
    if len(request.history) < minimum:
        raise LegacySourceGridNativeWave54SourceError(
...
def generate_legacy_source_grid_native_wave54_portfolio(
    request: LegacySourceGridNativeWave54Request,
) -> LegacySourceGridNativeWave54Result:
    ...
    randomness_used = RANDOMNESS_USED_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
    return LegacySourceGridNativeWave54Result(
        tickets=tickets,
        metadata=LegacySourceGridNativeWave54Metadata(
            protocol=SOURCE_NATIVE_WAVE54_PROTOCOL,
            legacy_method_id=method_id,
            source_sha256=SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id],
            ...
            random_protocol=RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id],
            ...
            source_history_order=(
                SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            source_history_order_detail=(
                SOURCE_HISTORY_ORDER_DETAIL_BY_SOURCE_NATIVE_WAVE54_METHOD[
                    method_id
                ]
            ),
            ...
            source_minimum_history_rationale=(
                MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            context_draw_count=len(request.history),
            context_numbers_sha256=context_sha256,
            candidate_k=None,
            source_candidate_k_values=(
                SOURCE_CANDIDATE_K_VALUES_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            native_ticket_count=len(tickets),
            native_ticket_count_semantics=(
                NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            native_ticket_order=(
            ...
            source_method_combination_count=(
                SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            combination_members=(
                SOURCE_CONFIGURATION_MEMBERS_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
            intra_ticket_order_semantics=(
                INTRA_TICKET_ORDER_SEMANTICS_BY_SOURCE_NATIVE_WAVE54_METHOD[method_id]
            ),
...
__all__ = [
    ...
    "SUPPORTED_SOURCE_NATIVE_WAVE54_METHODS",
    "LegacySourceGridNativeWave54Error",
    "LegacySourceGridNativeWave54Metadata",
