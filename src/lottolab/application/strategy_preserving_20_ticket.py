"""Outcome-blind deterministic ``strategy_preserving_20_ticket/v1`` port.

This is a target-native reproduction of the frozen P20 constructor.  Its
constants, seed preimage, candidate generation, objective, and parity digest
are versioned evidence; changing any of them requires a new constructor
version.
"""
...
class ConstructorFailureReason(StrEnum):
...
class ConstructionTier(StrEnum):
...
ConstructorResult = ConstructorSuccess | ConstructorFailure
...
def _normalise_ticket(raw: Sequence[int]) -> Ticket:
    ...
    if len(raw) != 6 or not all(type(number) is int for number in raw):
        raise ValueError("ticket must contain exactly six exact integers")
    values = tuple(sorted(raw))
    if len(set(values)) != 6 or any(number < 1 or number > 49 for number in values):
        raise ValueError("ticket numbers must be distinct and within 1..49")
...
def _normalise_explicit_signal(
    ...
    if number_scores is not None:
        for raw_number, raw_score in sorted(
            number_scores.items(),
            ...
            if type(raw_number) is not int or not 1 <= raw_number <= 49:
                ...
                continue
            if type(raw_score) not in (int, float) or not math.isfinite(
                float(raw_score)
            ):
    ...
    if ranked_numbers is not None:
        for raw_number in ranked_numbers:
            ...
            elif raw_number not in seen_ranked:
...
def _native_signal(tickets: Sequence[Ticket]) -> dict[int, float]:
    ...
    return {
        number: count / maximum for number, count in sorted(counts.items())
    }
...
def _sample(values: Sequence[int], count: int, stream: _Sha256Stream) -> list[int]:
    ...
    for upper in range(len(values) - count, len(values)):
        ...
        selected_indexes.add(
            upper if candidate in selected_indexes else candidate
        )
    ...
    *,
    ...
    while len(candidates) < desired and attempts < MAX_CANDIDATE_ATTEMPTS:
        ...
        if maximum_signal > minimum_signal:
            signal_count += stream.randbelow(
                maximum_signal - minimum_signal + 1
            )
        selected_signal = _sample(signal_pool, signal_count, stream)
        selected_set = set(selected_signal)
        remaining_pool = [
            number for number in universe if number not in selected_set
        ]
        selected_neutral = _sample(
            remaining_pool,
            6 - signal_count,
            stream,
        )
        values = sorted(selected_signal + selected_neutral)
        ticket: Ticket = (
            values[0],
            values[1],
            values[2],
    ...
    *,
    ...
    for _ in range(count):
        ...
        for ticket in remaining:
            ...
            maximum_overlap = max(
                (
                    _overlap_count(mask, other_mask)
            ...
            signal_score = sum(
                signal_scores.get(number, 0.0) for number in ticket
            )
            objective = (
...
def _construct(request: ConstructorRequest) -> ConstructorResult:
    ...
    if not _cutoff_is_valid(
        request.draw_id,
        request.historical_cutoff_identity,
    ):
        return _failure(
            request,
    ...
    raw_tickets_value: object = request.raw_tickets
    if isinstance(raw_tickets_value, str | bytes):
        return _failure(
            request,
    ...
    explicit_scores, explicit_source, signal_warnings = (
        _normalise_explicit_signal(
    ...
    if (
        len(tickets) != DEFAULT_TARGET_TICKET_COUNT
    ...
    if constructed:
        warnings.append(
            "constructed tickets use a deterministic neutral 1..49 universe "
            "for non-signal positions"
        )
    metadata = ConstructorMetadata(
        constructor_name=CONSTRUCTOR_NAME,
        constructor_version=CONSTRUCTOR_VERSION,
...
def construct_strategy_preserving_20_ticket(
    ...
    """Construct a closed typed result without target outcome access."""
