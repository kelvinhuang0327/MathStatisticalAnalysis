"""Causal materialization for the wave-58 dual/seeded predictors."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_DUAL_SEEDED_NATIVE_WAVE58_BATCH_V1"
)


class LegacyDualSeededNativeWave58BatchImportError(ValueError):
    """The pinned dataset or wave-58 ledger violates the batch contract."""
...
def _validate_logical_dataset_identity(
    ...
    if (
        ...
        raise LegacyDualSeededNativeWave58BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE58_METHODS:
        ...
        if (
            ...
            raise LegacyDualSeededNativeWave58BatchImportError(
                "frozen wave-58 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_dual_seeded_native_wave58_batch(
    *,
    database: Path,
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-dual-seeded-native-wave58-"
        ...
        "source_provenance": {
            ...
            "logical_history_anchor": (
                "CHECKSUMMED_WAVE58_ALL_TARGET_FULL_PREFIX_CONTEXT_LEDGER"
            ),
            ...
            "source_candidate_k_values": {
                method_id: list(values)
                for method_id, values in (
                    SOURCE_CANDIDATE_K_VALUES_BY_SOURCE_NATIVE_WAVE58_METHOD.items()
                )
            },
