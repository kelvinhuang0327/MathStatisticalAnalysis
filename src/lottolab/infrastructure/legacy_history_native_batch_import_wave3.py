"""Causal materialization for the third frozen history-native BIG_LOTTO batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_HISTORY_NATIVE_WAVE3_BATCH_V1"
)


class LegacyHistoryNativeWave3BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_HISTORY_NATIVE_WAVE3_METHODS:
        ...
        if (
            ...
            raise LegacyHistoryNativeWave3BatchImportError(
                "frozen history-native wave-3 catalog identity changed"
            )
        records.append(record)
    return tuple(records)


def materialize_legacy_history_native_wave3_batch(
    *,
    database: Path,
    ...
    try:
        ...
        raise LegacyHistoryNativeWave3BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            minimum_history = (
                MINIMUM_HISTORY_BY_HISTORY_NATIVE_WAVE3_METHOD[method_id]
            )
            ...
            try:
                native = generate_legacy_history_native_wave3_portfolio(
                    LegacyHistoryNativeWave3Request(
            ...
            except LegacyHistoryNativeWave3SourceError as exc:
                raise LegacyHistoryNativeWave3BatchImportError(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-history-native-wave3-"
