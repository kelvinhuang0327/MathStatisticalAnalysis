"""Causal materialization for the wave-57 frozen HPSB-V2 predictor."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_HPSB_NATIVE_WAVE57_BATCH_V1"
)
NO_CAUSAL_CUTOFF_REASON = "NO_PRIOR_DRAW_FOR_CAUSAL_CUTOFF"


class LegacyHpsbNativeWave57BatchImportError(ValueError):
    """The pinned dataset or HPSB ledger violates the batch contract."""
...
def _validate_logical_dataset_identity(
    ...
    if (
        ...
        raise LegacyHpsbNativeWave57BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE57_METHODS:
        ...
        if (
            ...
            raise LegacyHpsbNativeWave57BatchImportError(
                "frozen HPSB-native wave-57 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_hpsb_native_wave57_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE57_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 input for every target with a prior causal draw."""
    ...
    try:
        ...
        raise LegacyHpsbNativeWave57BatchImportError(str(exc)) from exc
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE57_METHODS:
        ...
        for target_index, target in enumerate(pinned_history.draws):
            if target_index == 0:
                executions.append(
                    {
                        "reason_code": NO_CAUSAL_CUTOFF_REASON,
            ...
            native = generate_legacy_hpsb_native_wave57_portfolio(
                LegacyHpsbNativeWave57Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-hpsb-native-wave57-"
        ...
        "source_provenance": {
            ...
            "candidate_k_semantics": (
                ...
                "FROM_SOURCE_CANDIDATE_K_NATIVE_TICKET_COUNT_LOCAL_"
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE57_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE57_METHODS
            },
            ...
            "logical_history_anchor": (
                "CHECKSUMMED_WAVE57_ALL_TARGET_FULL_PREFIX_CONTEXT_LEDGER"
            ),
            "native_ticket_count": {
                method_id: (
                    NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE57_METHOD[
            ...
            "random_protocols": {
                method_id: RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE57_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE57_METHODS
            },
...
__all__ = [
    ...
    "LegacyHpsbNativeWave57BatchImportError",
    "materialize_legacy_hpsb_native_wave57_batch",
]
