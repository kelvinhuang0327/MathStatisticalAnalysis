"""Causal materialization for the wave-46 frozen source-grid batch."""
...
MATERIALIZATION_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE46_BATCH_V1"
CLOSED_REASON = "AVAILABLE_HISTORY_BELOW_FROZEN_SOURCE_MINIMUM"


class LegacySourceGridNativeWave46BatchImportError(ValueError):
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE46_METHODS:
        ...
        if (
            ...
            raise LegacySourceGridNativeWave46BatchImportError(
                "frozen source-grid wave-46 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_source_grid_native_wave46_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE46_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for twelve executable source rows."""
    ...
    try:
        ...
        raise LegacySourceGridNativeWave46BatchImportError(str(exc)) from exc
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE46_METHODS:
        ...
        minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE46_METHOD[method_id]
        ...
        for target_index, target in enumerate(pinned_history.draws):
            ...
            native = generate_legacy_source_grid_native_wave46_portfolio(
                LegacySourceGridNativeWave46Request(
    ...
    return {
        "dataset_id": (
            "legacy-biglotto-source-grid-native-wave46-"
        ...
        "source_provenance": {
            "alias_disposition": {
                "alias_method": PREDICTABILITY_ALIAS_METHOD_ID,
                "canonical_method": OPTIMAL_MATRIX_METHOD_ID,
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE46_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE46_METHODS
            },
            ...
            "minimum_history_draws": {
                method_id: MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE46_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE46_METHODS
            },
            "native_ticket_count": {
                method_id: NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE46_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE46_METHODS
            },
