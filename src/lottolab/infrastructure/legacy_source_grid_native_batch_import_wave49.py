"""Causal materialization for the wave-49 frozen source-grid batch."""
...
MATERIALIZATION_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE49_BATCH_V1"
CLOSED_REASON = "AVAILABLE_HISTORY_BELOW_FROZEN_SOURCE_MINIMUM"


class LegacySourceGridNativeWave49BatchImportError(ValueError):
...
def _validate_logical_dataset_identity(
    ...
    ledger = load_legacy_source_grid_native_wave49_ledger_for_verification()
    ...
    if (
        ...
        raise LegacySourceGridNativeWave49BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS:
        ...
        if (
            record is None
            or record.source_sha256 != SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
        ):
            raise LegacySourceGridNativeWave49BatchImportError(
                "frozen source-grid wave-49 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_source_grid_native_wave49_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE49_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for three executable source rows."""
    ...
    try:
        ...
        raise LegacySourceGridNativeWave49BatchImportError(str(exc)) from exc
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS:
        ...
        minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
        ...
        for target_index, target in enumerate(pinned_history.draws):
            ...
            native = generate_legacy_source_grid_native_wave49_portfolio(
                LegacySourceGridNativeWave49Request(
    ...
    return {
        "dataset_id": f"legacy-biglotto-source-grid-native-wave49-{logical_dataset_sha256[:12]}",
        "dataset_sha256": logical_dataset_sha256,
        ...
        "source_provenance": {
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
            "intra_ticket_order_semantics": {
                method_id: INTRA_TICKET_ORDER_SEMANTICS_BY_SOURCE_NATIVE_WAVE49_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
            ...
            "logical_history_anchor": "CHECKSUMMED_WAVE49_FULL_PREFIX_CONTEXT_LEDGER",
            "native_ticket_count": {
                method_id: NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
            "native_ticket_count_semantics": {
                method_id: NATIVE_TICKET_SEMANTICS_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
            "random_protocol": {
            ...
            "source_configuration_count": {
                method_id: SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
            ...
            "source_minimum_history_rationale": {
                method_id: MINIMUM_HISTORY_RATIONALE_BY_SOURCE_NATIVE_WAVE49_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS
            },
...
__all__ = [
    ...
    "LegacySourceGridNativeWave49BatchImportError",
    "materialize_legacy_source_grid_native_wave49_batch",
]
