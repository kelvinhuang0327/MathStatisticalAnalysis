"""Causal materialization for the wave-50 frozen source-grid batch."""
...
MATERIALIZATION_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE50_BATCH_V1"
CLOSED_REASON = "AVAILABLE_HISTORY_BELOW_FROZEN_SOURCE_MINIMUM"


class LegacySourceGridNativeWave50BatchImportError(ValueError):
...
def _validate_logical_dataset_identity(
    ...
    ledger = load_legacy_source_grid_native_wave50_ledger_for_verification()
    ...
    if (
        ...
        raise LegacySourceGridNativeWave50BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS:
        ...
        if (
            ...
            raise LegacySourceGridNativeWave50BatchImportError(
                "frozen source-grid wave-50 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_source_grid_native_wave50_batch(
    *,
    database: Path,
    ...
    try:
        ...
        raise LegacySourceGridNativeWave50BatchImportError(str(exc)) from exc
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS:
        ...
        minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE50_METHOD[method_id]
        ...
        for target_index, target in enumerate(pinned_history.draws):
            ...
            native = generate_legacy_source_grid_native_wave50_portfolio(
                LegacySourceGridNativeWave50Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-grid-native-wave50-"
        ...
        "source_provenance": {
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE50_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS
            },
            ...
            "random_protocol": {
                method_id: RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE50_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS
            },
