"""Causal materialization for the wave-52 frozen source-grid batch."""
...
MATERIALIZATION_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE52_BATCH_V1"
CLOSED_REASON = "AVAILABLE_HISTORY_BELOW_FROZEN_SOURCE_MINIMUM"


class LegacySourceGridNativeWave52BatchImportError(ValueError):
...
def _validate_logical_dataset_identity(
    ...
    ledger = load_legacy_source_grid_native_wave52_ledger_for_verification()
    ...
    if (
        ...
        raise LegacySourceGridNativeWave52BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS:
        ...
        if (
            ...
            raise LegacySourceGridNativeWave52BatchImportError(
                "frozen source-grid wave-52 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_source_grid_native_wave52_batch(
    *,
    database: Path,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS:
        ...
        minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE52_METHOD[method_id]
        ...
        for target_index, target in enumerate(pinned_history.draws):
            ...
            native = generate_legacy_source_grid_native_wave52_portfolio(
                LegacySourceGridNativeWave52Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-grid-native-wave52-"
        ...
        "source_provenance": {
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE52_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS
            },
            ...
            "random_protocol": {
                method_id: RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE52_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS
            },
            ...
            "source_history_order": {
                method_id: SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE52_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS
            },
