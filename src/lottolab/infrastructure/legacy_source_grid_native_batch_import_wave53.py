"""Causal materialization for the wave-53 frozen source-grid batch."""
...
MATERIALIZATION_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE53_BATCH_V1"
CLOSED_REASON = "AVAILABLE_HISTORY_BELOW_FROZEN_SOURCE_MINIMUM"


class LegacySourceGridNativeWave53BatchImportError(ValueError):
...
def _validate_logical_dataset_identity(
    ...
    ledger = load_legacy_source_grid_native_wave53_ledger_for_verification()
    ...
    if (
        ...
        raise LegacySourceGridNativeWave53BatchImportError(
...
def _catalog_records() -> dict[str, FullStrategyCatalogRecord]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS:
        ...
        if (
            ...
            raise LegacySourceGridNativeWave53BatchImportError(
                "frozen source-grid wave-53 catalog identity changed"
            )
        records[method_id] = record
    return records


def materialize_legacy_source_grid_native_wave53_batch(
    *,
    database: Path,
    ...
    try:
        ...
        raise LegacySourceGridNativeWave53BatchImportError(str(exc)) from exc
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS:
        ...
        minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
        ...
        for target_index, target in enumerate(pinned_history.draws):
            ...
            native = generate_legacy_source_grid_native_wave53_portfolio(
                LegacySourceGridNativeWave53Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-grid-native-wave53-"
        ...
        "source_provenance": {
            ...
            "frozen_sources": {
                method_id: SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS
            },
            ...
            "random_protocol": {
                method_id: RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS
            },
            ...
            "source_history_order": {
                method_id: SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE53_METHOD[
                    method_id
                ]
                for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS
            },
...
__all__ = [
    ...
    "LegacySourceGridNativeWave53BatchImportError",
    "materialize_legacy_source_grid_native_wave53_batch",
]
