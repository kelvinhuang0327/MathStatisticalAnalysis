"""Causal materialization for the eleventh frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE11_BATCH_V1"
)


class LegacySourceNativeWave11BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE11_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave11BatchImportError(
                "frozen source-native wave-11 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave11_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE11_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for two source-native methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave11BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            try:
                native = generate_legacy_source_native_wave11_portfolio(
                    LegacySourceNativeWave11Request(
            ...
            except LegacySourceNativeWave11SourceError as exc:
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave11-"
