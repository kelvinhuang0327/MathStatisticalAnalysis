"""Causal materialization for the seventeenth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE17_BATCH_V1"
)


class LegacySourceNativeWave17BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE17_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave17BatchImportError(
                "frozen source-native wave-17 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave17_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE17_USER_SEED,
) -> dict[str, object]:
    """Build target-major ordered-20 evaluator input for both wave-17 methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave17BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE17_METHOD[
                method_id
            ]
            ...
            try:
                native = generate_legacy_source_native_wave17_portfolio(
                    LegacySourceNativeWave17Request(
            ...
            except LegacySourceNativeWave17SourceError as exc:
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave17-"
        ...
        "source_provenance": {
            ...
            "candidate_k_semantics": (
                ...
                "SMART_MULTI_CATEGORY_POOL_SIZES_REMAIN_SEPARATE"
            ),
            "combination_count": dict(
                SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE17_METHOD
            ),
            "combination_count_semantics": {
                SUPPORTED_SOURCE_NATIVE_WAVE17_METHODS[0]: (
                    "ONE_SOURCE_CONFIGURATION_DISTINCT_FROM_SEVEN_"
                    "NATIVE_TICKETS"
                ),
                SUPPORTED_SOURCE_NATIVE_WAVE17_METHODS[1]: (
                    "SIX_SOURCE_STRATEGY_BRANCHES_DISTINCT_FROM_SIX_"
                    "NATIVE_TICKETS"
                ),
            },
            ...
            "execution_status_counts_by_method": {
                method_id: dict(sorted(counts.items()))
                for method_id, counts in status_counts_by_method.items()
            },
            "frozen_sources": dict(
                SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE17_METHOD
            ),
            ...
            "source_result_selection": {
                SUPPORTED_SOURCE_NATIVE_WAVE17_METHODS[0]: (
                    "NORMATIVE_DIVERSE_RANDOM_THEN_STABLE_EV_DESCENDING"
                ),
                SUPPORTED_SOURCE_NATIVE_WAVE17_METHODS[1]: (
                    "SIX_COMPLEMENTARY_POOL_STRATEGIES_IN_SOURCE_ORDER"
                ),
            },
            "user_seed": user_seed,
        },
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave17BatchImportError",
    "materialize_legacy_source_native_wave17_batch",
]
