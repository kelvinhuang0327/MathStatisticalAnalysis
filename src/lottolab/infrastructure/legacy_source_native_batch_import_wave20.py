"""Causal materialization for the twentieth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE20_BATCH_V1"
)


class LegacySourceNativeWave20BatchImportError(ValueError):
...
def _catalog_record() -> FullStrategyCatalogRecord:
    ...
    try:
        record = next(
            ...
            if record.legacy_method_id == ZONE_BALANCE_500_METHOD_ID
        )
    except StopIteration as exc:
        raise LegacySourceNativeWave20BatchImportError(
            "frozen source-native wave-20 catalog identity changed"
        ) from exc
...
def materialize_legacy_source_native_wave20_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE20_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for the zone-balance source."""
    ...
    try:
        ...
        raise LegacySourceNativeWave20BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        try:
            native = generate_legacy_source_native_wave20_portfolio(
                LegacySourceNativeWave20Request(
        ...
        except LegacySourceNativeWave20SourceError as exc:
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave20-"
        ...
        "source_provenance": {
            ...
            "combination_count_semantics": (
                "FOUR_DISTINCT_WINDOW_CONFIGURATIONS_SEPARATE_FROM_"
            ...
            "execution_status_counts_by_method": {
                ZONE_BALANCE_500_METHOD_ID: dict(
                    sorted(status_counts.items())
                )
            },
            "frozen_sources": dict(
            ...
            "source_result_selection": (
                "PRESERVE_MAIN_500_RECOMMENDATION_THEN_FOUR_PRINTED_"
                "WINDOW_COMPARISON_TICKETS_WITH_POSITIONAL_DUPLICATE"
            ),
            "user_seed": user_seed,
        },
        "targets": targets,
