"""Causal materialization for the twenty-first frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE21_BATCH_V1"
)


class LegacySourceNativeWave21BatchImportError(ValueError):
...
def _catalog_record() -> FullStrategyCatalogRecord:
    ...
    try:
        record = next(
            ...
            if record.legacy_method_id == POST_SELECTION_FILTER_METHOD_ID
        )
    except StopIteration as exc:
        raise LegacySourceNativeWave21BatchImportError(
            "frozen source-native wave-21 catalog identity changed"
        ) from exc
...
def materialize_legacy_source_native_wave21_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE21_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for post-selection filtering."""
    ...
    try:
        ...
        raise LegacySourceNativeWave21BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        try:
            native = generate_legacy_source_native_wave21_portfolio(
                LegacySourceNativeWave21Request(
        ...
        except LegacySourceNativeWave21SourceError as exc:
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave21-"
        ...
        "source_provenance": {
            ...
            "execution_status_counts_by_method": {
                POST_SELECTION_FILTER_METHOD_ID: dict(
                    sorted(status_counts.items())
                )
            },
            "frozen_sources": dict(
            ...
            "source_result_selection": {
                POST_SELECTION_FILTER_METHOD_ID: (
                    "FREQUENCY_50_POST_FILTER_FIRST_THEN_ZONE_BALANCE_"
