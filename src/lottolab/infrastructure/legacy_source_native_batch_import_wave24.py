"""Causal materialization for the twenty-fourth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE24_BATCH_V1"
)


class LegacySourceNativeWave24BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE24_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave24BatchImportError(
                "frozen source-native wave-24 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave24_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE24_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for six frozen candidate-pool methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave24BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE24_METHOD[
                method_id
            ]
            if len(prior_history) < minimum:
                raise LegacySourceNativeWave24BatchImportError(
                    "wave-24 minimum-history contracts diverged"
                )
            try:
                native = build_legacy_source_native_wave24_result(
                    LegacySourceNativeWave24Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave24-"
        ...
        "source_provenance": {
            ...
            "candidate_k_semantics": (
                "EXECUTION_SPECIFIC_LENGTH_OF_FROZEN_RANKED_NUMBER_POOL_"
                "DISTINCT_FROM_NATIVE_TICKET_AND_METHOD_COMBINATION_COUNTS"
            ),
            "combination_count": dict(
                SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE24_METHOD
            ),
            "combination_count_semantics": (
                "WEIGHTED_UNIFIED_PREDICTOR_MEMBER_COUNT_DISTINCT_FROM_"
            ...
            "source_native_protocol": SOURCE_NATIVE_WAVE24_PROTOCOL,
            "source_read_mode": (
