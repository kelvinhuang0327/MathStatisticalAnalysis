"""Causal materialization for the twenty-fifth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE25_BATCH_V1"
)


class LegacySourceNativeWave25BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE25_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave25BatchImportError(
                "frozen source-native wave-25 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave25_batch(
    *,
    database: Path,
    ...
    try:
        ...
        raise LegacySourceNativeWave25BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE25_METHOD[
                method_id
            ]
            if len(prior_history) < minimum:
                raise LegacySourceNativeWave25BatchImportError(
                    "wave-25 minimum-history contracts diverged"
                )
            try:
                native = build_legacy_source_native_wave25_result(
                    LegacySourceNativeWave25Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave25-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                "EXECUTION_SPECIFIC_FROZEN_RANKED_POOL_LENGTH_OR_NULL_"
                "WHEN_SOURCE_HAS_NO_PRE_TICKET_CANDIDATE_POOL"
            ),
            "combination_count": dict(
                SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE25_METHOD
            ),
            "combination_count_semantics": (
                "SOURCE_UNIFIED_PREDICTOR_MEMBER_COUNT_DISTINCT_FROM_"
            ...
            "source_native_protocol": SOURCE_NATIVE_WAVE25_PROTOCOL,
            "source_read_mode": (
