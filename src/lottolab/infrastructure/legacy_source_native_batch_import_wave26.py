"""Causal materialization for the twenty-sixth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE26_BATCH_V1"
)
_ENGINE_METHODS = (
    "frequency",
    "bayesian",
...
class LegacySourceNativeWave26BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE26_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave26BatchImportError(
                "frozen source-native wave-26 catalog identity changed"
            )
    ...
    for method_name in _ENGINE_METHODS:
        try:
            cache[(len(history), method_name)] = (
                frozen_wave26_engine_output(method_name, history)
            )
        except Exception:
            continue


def materialize_legacy_source_native_wave26_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE26_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for five frozen portfolio methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave26BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE26_METHOD[
                method_id
            ]
            ...
            try:
                native = generate_legacy_source_native_wave26_portfolio(
                    LegacySourceNativeWave26Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave26-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                "EXECUTION_SPECIFIC_FROZEN_CANDIDATE_POOL_LENGTH_OR_NULL_"
                "WHEN_SOURCE_EXPOSES_NO_PRE_TICKET_POOL"
            ),
            ...
            "source_native_protocol": SOURCE_NATIVE_WAVE26_PROTOCOL,
            "source_read_mode": (
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave26BatchImportError",
