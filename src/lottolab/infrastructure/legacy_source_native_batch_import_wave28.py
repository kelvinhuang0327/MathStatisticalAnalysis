"""Causal materialization for the twenty-eighth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE28_BATCH_V1"
)
_ENGINE_METHODS = (
    "deviation",
    "markov",
...
class LegacySourceNativeWave28BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE28_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave28BatchImportError(
                "frozen source-native wave-28 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave28_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE28_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for three frozen portfolio methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave28BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        if prior_history:
            _cache_source_history(
                tuple(reversed(prior_history)),
                cache,
            )
        for record in records:
            method_id = record.legacy_method_id
            minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE28_METHOD[
                method_id
            ]
            ...
            try:
                native = generate_legacy_source_native_wave28_portfolio(
                    LegacySourceNativeWave28Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave28-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                "EXECUTION_SPECIFIC_FROZEN_RANKED_CANDIDATE_POOL_LENGTH_"
                "OR_NULL_FOR_ELITE_WINDOW_CONSENSUS"
            ),
            ...
            "declared_native_ticket_count": dict(
                DECLARED_NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE28_METHOD
            ),
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave28BatchImportError",
