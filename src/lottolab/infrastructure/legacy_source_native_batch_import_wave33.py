"""Causal materialization for the thirty-third frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE33_BATCH_V1"
)


class LegacySourceNativeWave33BatchImportError(ValueError):
...
def _catalog_record() -> FullStrategyCatalogRecord:
    ...
    record = by_method_id.get(FEASIBILITY_METHOD_ID)
    if (
        record is None
        ...
        raise LegacySourceNativeWave33BatchImportError(
            "frozen source-native wave-33 catalog identity changed"
        )
    return record


def materialize_legacy_source_native_wave33_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE33_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for the feasibility method."""
    ...
    try:
        ...
        raise LegacySourceNativeWave33BatchImportError(str(exc)) from exc
    ...
    minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE33_METHOD[
        FEASIBILITY_METHOD_ID
    ]
    for target_index, target in enumerate(pinned_history.draws):
        ...
        try:
            native = generate_legacy_source_native_wave33_portfolio(
                LegacySourceNativeWave33Request(
        ...
        except LegacySourceNativeWave33SourceError as exc:
            executions.append(
                {
            ...
            status_counts["CLOSED_INVALID_OUTPUT"] += 1
            continue
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave33-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                "NULL_NO_COMBINATORIAL_CANDIDATE_K_WHILE_TOP12_"
                "COUNTER_POOL_IS_RETAINED_SEPARATELY"
            ),
            "combination_count": dict(
                SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE33_METHOD
            ),
            "combination_count_semantics": (
                "SOURCE_BENCHMARK_CONFIGURATION_COUNT_DISTINCT_FROM_"
            ...
            "random_protocols": {
                FEASIBILITY_METHOD_ID: (
                    "WRAPPER_RANDOM_AND_NUMPY_SEED_42_BEFORE_EACH_"
                    "BENCHMARK_WITH_STATISTICAL_RANDOM_RESEEDED_BY_"
                    "HISTORY_LENGTH"
                )
            },
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave33BatchImportError",
    "materialize_legacy_source_native_wave33_batch",
]
