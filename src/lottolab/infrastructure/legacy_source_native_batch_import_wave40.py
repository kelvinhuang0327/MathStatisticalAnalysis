"""Causal materialization for the fortieth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE40_BATCH_V1"
)


class LegacySourceNativeWave40BatchImportError(ValueError):
...
def _catalog_record() -> FullStrategyCatalogRecord:
    ...
    record = by_method_id.get(PORTFOLIO_METHOD_ID)
    if (
        record is None
        ...
        raise LegacySourceNativeWave40BatchImportError(
            "frozen source-native wave-40 catalog identity changed"
        )
    return record


def materialize_legacy_source_native_wave40_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE40_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for the deterministic 3+1 portfolio."""
    ...
    try:
        ...
        raise LegacySourceNativeWave40BatchImportError(str(exc)) from exc
    ...
    minimum = MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE40_METHOD[
        PORTFOLIO_METHOD_ID
    ]
    for target_index, target in enumerate(pinned_history.draws):
        ...
        native = generate_legacy_source_native_wave40_portfolio(
            LegacySourceNativeWave40Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave40-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                ...
                "THREE_SOURCE_COMPONENTS_VARIABLE_NATIVE_TICKETS_AND_"
                "ORDERED20_COUNT"
            ),
            ...
            "combination_count_semantics": (
                "THREE_SOURCE_PORTFOLIO_COMPONENTS_DISTINCT_FROM_"
            ...
            "execution_status_counts_by_method": {
                PORTFOLIO_METHOD_ID: dict(sorted(status_counts.items()))
            },
            "frozen_sources": dict(
                SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE40_METHOD
            ),
            ...
            "native_ticket_count_upper_bound": dict(
                NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE40_METHOD
            ),
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave40BatchImportError",
