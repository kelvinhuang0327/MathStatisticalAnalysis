"""Causal materialization for the forty-first frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE41_BATCH_V1"
)


class LegacySourceNativeWave41BatchImportError(ValueError):
...
def _catalog_record() -> FullStrategyCatalogRecord:
    ...
    record = by_method_id.get(GRAPH_METHOD_ID)
    if (
        record is None
        ...
        raise LegacySourceNativeWave41BatchImportError(
            "frozen source-native wave-41 catalog identity changed"
        )
    return record


def materialize_legacy_source_native_wave41_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE41_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for graph and deviation tickets."""
    ...
    try:
        ...
        raise LegacySourceNativeWave41BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        native = generate_legacy_source_native_wave41_portfolio(
            LegacySourceNativeWave41Request(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave41-"
        ...
        "source_provenance": {
            "candidate_k_semantics": (
                "NULL_NO_TRUNCATED_CANDIDATE_K_DISTINCT_FROM_TWO_"
                "SOURCE_CONFIGURATIONS_NATIVE_TICKETS_AND_ORDERED20_COUNT"
            ),
            "combination_count": dict(
                SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE41_METHOD
            ),
            "combination_count_semantics": (
                "TWO_SOURCE_METHOD_CONFIGURATIONS_DISTINCT_FROM_"
            ...
            "execution_status_counts_by_method": {
                GRAPH_METHOD_ID: dict(sorted(status_counts.items()))
            },
            "frozen_sources": dict(
                SOURCE_SHA256_BY_SOURCE_NATIVE_WAVE41_METHOD
            ),
            ...
            "random_protocols": {
                GRAPH_METHOD_ID: "NONE_DETERMINISTIC"
            },
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave41BatchImportError",
