"""Causal materialization for the sixth frozen source-native BIG_LOTTO batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE6_BATCH_V1"
)


class LegacySourceNativeWave6BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE6_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave6BatchImportError(
                "frozen source-native wave-6 catalog identity changed"
            )
        records.append(record)
    return tuple(records)


def materialize_legacy_source_native_wave6_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE6_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for four more source-native methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave6BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            try:
                native = generate_legacy_source_native_wave6_portfolio(
                    LegacySourceNativeWave6Request(
            ...
            except LegacySourceNativeWave6SourceError as exc:
                raise LegacySourceNativeWave6BatchImportError(
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave6-"
