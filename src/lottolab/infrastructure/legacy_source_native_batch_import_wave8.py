"""Causal materialization for the eighth frozen source-native batch."""
...
MATERIALIZATION_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE8_BATCH_V1"
)


class LegacySourceNativeWave8BatchImportError(ValueError):
...
def _catalog_records() -> tuple[FullStrategyCatalogRecord, ...]:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE8_METHODS:
        ...
        if (
            ...
            raise LegacySourceNativeWave8BatchImportError(
                "frozen source-native wave-8 catalog identity changed"
            )
...
def materialize_legacy_source_native_wave8_batch(
    *,
    database: Path,
    expected_database_sha256: str,
    user_seed: str | int = DEFAULT_SOURCE_NATIVE_WAVE8_USER_SEED,
) -> dict[str, object]:
    """Build ordered-20 evaluator input for four source-native methods."""
    ...
    try:
        ...
        raise LegacySourceNativeWave8BatchImportError(str(exc)) from exc
    ...
    for target_index, target in enumerate(pinned_history.draws):
        ...
        for record in records:
            ...
            try:
                native = generate_legacy_source_native_wave8_portfolio(
                    LegacySourceNativeWave8Request(
            ...
            except LegacySourceNativeWave8SourceError as exc:
            ...
            if method_id == OPTIMIZE_THIRD_BET_METHOD_ID:
                candidate_k = (
                    native.metadata.source_candidate_ticket_counts[0]
                )
                execution["candidate_k"] = candidate_k
                execution["candidate_combination_count"] = (
                    math.comb(candidate_k, 6)
                )
    ...
    return {
        "dataset_id": (
            f"legacy-biglotto-source-native-wave8-"
        ...
        "source_provenance": {
            ...
            "candidate_k_semantics": (
                "EXECUTION_LEVEL_ONLY_FOR_OPTIMIZE_THIRD_BET_"
                "SEARCH_POOL_DISTINCT_FROM_TICKET_COUNT"
            ),
...
__all__ = [
    "MATERIALIZATION_SCHEMA_VERSION",
    "LegacySourceNativeWave8BatchImportError",
