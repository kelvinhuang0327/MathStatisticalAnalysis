"""Builds the P638 all-10 official-prize historical ranking projection.
...
must never be mutated): this module runs a fresh causal replay of all 10
currently executable POWER_LOTTO strategies against the reconciled
1,933-draw authority, evaluates every complete ticket under the official
POWER_LOTTO prize-tier table (not the 8-strategy contract's
``combined_zone1_4plus_zone2_hit_count`` heuristic), ranks all 10 by
historical winning-target rate, and writes the
``P638_HISTORICAL_RESULTS_ALL10_PRIZE_RANKING_V1`` projection.
...
class P638All10RankingBuildError(RuntimeError):
    """The P638 all-10 replay/ranking build failed reconciliation."""


@dataclass(frozen=True, slots=True)
class P638All10RankingBuildResult:
    run_id: str
...
def _load_draws_from_draw_db(draw_db: Path) -> tuple[tuple[PowerLottoDrawRecord, ...], str]:
    ...
    try:
        ...
        if migration is None:
            raise P638All10RankingBuildError(
        ...
        if not rows:
            raise P638All10RankingBuildError("draw database has no POWER_LOTTO draws")
...
def _compute_ranking(
    *,
    ...
    return P638RankingRecord(
        run_id=run_id,
        rank=0,  # assigned by the caller after sorting all 10
    ...
    with open_database(output_db) as connection:
        ...
        try:
            ...
            for spec in WAVE1_STRATEGIES:
                connection.execute(
                    """
                    INSERT INTO p638_all10_strategy (
            ...
            for spec in WAVE1_STRATEGIES:
                for target in targets_by_strategy.get(spec.strategy_id, ()):
                    ...
                    target_id = f"p638-all10-target-{digest}"
                    target_ids[(target.strategy_id, target.target_draw_number)] = target_id
                    connection.execute(
                        """
                        INSERT INTO p638_all10_target (
            ...
            for spec in WAVE1_STRATEGIES:
                ...
                for ticket in tickets_by_strategy.get(spec.strategy_id, ()):
                    ...
                    if target_id is None:
                        raise P638All10RankingBuildError(
                            "P638 all-10 ticket has no target row"
                        )
                    ticket_id = (
                        f"p638-all10-ticket-"
                ...
                for target_draw_number, target_id in (
                    ...
                    connection.execute(
                        "UPDATE p638_all10_target SET target_is_winner = ? WHERE id = ?",
            ...
            for record in rankings:
                connection.execute(
                    """
                    INSERT INTO p638_all10_ranking (
            ...
            if connection.execute("PRAGMA foreign_key_check").fetchall():
                raise P638All10RankingBuildError(
                    "P638 all-10 ranking projection left foreign-key violations"
                )
...
def build_p638_all10_ranking(
    *,
    draw_db: Path,
    runtime_root: Path,
    output_db: Path,
    replay_db_path: Path | None = None,
) -> P638All10RankingBuildResult:
    """Run the all-10 causal replay and build the official-prize ranking projection."""
    ...
    if replay_result.failed_target_count != 0:
        raise P638All10RankingBuildError(
            "P638 all-10 replay produced failed targets: "
            f"{replay_result.failed_target_count}"
        )
    if replay_result.selected_count != len(WAVE1_STRATEGIES):
        raise P638All10RankingBuildError(
            "P638 all-10 replay did not select all 10 executable strategies"
        )
    ...
    return P638All10RankingBuildResult(
        run_id=replay_result.run_id,
        contract_version=CONTRACT_VERSION,
        source_replay_db_sha256=replay_db_sha256,
        source_draw_db_sha256=draw_db_sha256,
        draw_count=len(draws),
        strategy_count=len(WAVE1_STRATEGIES),
