"""Dedicated, versioned, idempotent SQLite schema for the P638 all-10 prize-ranking projection.

A standalone schema module, deliberately not sharing anything with
``historical_schema.py`` or the ``p638_historical_forwarder.py`` P638
Historical Results V2 contract it produces: that contract is frozen to one
specific 8-strategy source replay and must not be mutated by this task.
This module owns a distinct, separately versioned contract
(``P638_HISTORICAL_RESULTS_ALL10_PRIZE_RANKING_V1``) for the all-10
executable-strategy replay and official POWER_LOTTO prize-tier ranking.
Mirrors ``historical_schema.py``'s versioned-migration and
semantic-drift-detection pattern (BLHQ R1) at a scope appropriate to a
dedicated, task-owned database.
"""
...
MIGRATION_NAME = "create_p638_all10_ranking_schema"
BUSY_TIMEOUT_MS = 5_000

CONTRACT_VERSION = "P638_HISTORICAL_RESULTS_ALL10_PRIZE_RANKING_V1"

TABLE_NAMES = (
    "p638_all10_schema_migrations",
    "p638_all10_run",
    "p638_all10_strategy",
    "p638_all10_target",
    "p638_all10_ticket",
...
class P638All10RankingSchemaError(RuntimeError):
    """The P638 all-10 ranking database path or schema failed a safety check."""


class P638All10RankingSchemaMigrationError(P638All10RankingSchemaError):
    """The P638 all-10 ranking database schema is absent, corrupt, or drifted."""


class P638All10RankingSchemaChecksumError(P638All10RankingSchemaMigrationError):
    """A recorded migration does not match the code-owned migration."""


@dataclass(frozen=True, slots=True)
class P638All10RankingDatabasePaths:
...
MIGRATION_STATEMENTS = (
    """
    CREATE TABLE p638_all10_schema_migrations (
    ...
    CREATE TABLE p638_all10_run (
        ...
        strategy_count INTEGER NOT NULL CHECK (strategy_count = 10),
    ...
    """,
    """
    CREATE TABLE p638_all10_strategy (
        ...
        FOREIGN KEY (run_id) REFERENCES p638_all10_run(run_id) ON DELETE RESTRICT
    )
    """,
    """
    CREATE TABLE p638_all10_target (
        id TEXT PRIMARY KEY,
        run_id TEXT NOT NULL,
        strategy_id TEXT NOT NULL,
        strategy_version TEXT NOT NULL,
        ...
        FOREIGN KEY (run_id, strategy_id)
            REFERENCES p638_all10_strategy(run_id, strategy_id) ON DELETE RESTRICT
    )
    """,
    """
    CREATE TABLE p638_all10_ticket (
        id TEXT PRIMARY KEY,
        ...
        FOREIGN KEY (target_id) REFERENCES p638_all10_target(id) ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE p638_all10_ranking (
        run_id TEXT NOT NULL,
        strategy_id TEXT NOT NULL,
    ...
    """,
    """
    CREATE INDEX idx_p638_all10_target_query
    ON p638_all10_target (run_id, strategy_id, target_draw_date, target_draw_number, status)
    """,
    """
    CREATE INDEX idx_p638_all10_ticket_query
    ON p638_all10_ticket (run_id, strategy_id, target_draw_number, ticket_position)
    """,
    """
    CREATE INDEX idx_p638_all10_ranking_rank
    ON p638_all10_ranking (run_id, rank)
    """,
)
...
def _object_name(sql: str) -> str:
    ...
    if match is None:
        raise P638All10RankingSchemaError("cannot determine schema object name")
...
def resolve_p638_all10_ranking_database_paths(database: Path) -> P638All10RankingDatabasePaths:
    """Validate an explicit, caller-owned database path. Never creates anything."""

    if "\x00" in str(database):
        raise P638All10RankingSchemaError("database path contains a null byte")
    if not database.is_absolute():
        raise P638All10RankingSchemaError("database path must be absolute")
    if any(part.casefold() == "lotterynew" for part in database.parts):
        raise P638All10RankingSchemaError("LotteryNew paths are forbidden")
    return P638All10RankingDatabasePaths(database=database)


def initialize_schema(database: Path) -> None:
    """Securely create a P638 all-10 ranking database.

    Idempotent: a call against an already-initialized current database is a
    read-only semantic verification, not a rewrite.
    """

    paths = resolve_p638_all10_ranking_database_paths(database)
    paths.database.parent.mkdir(parents=True, exist_ok=True)
    with _raw_connection(paths) as connection:
        try:
            connection.execute("PRAGMA foreign_keys = OFF")
            if connection.execute("PRAGMA foreign_keys").fetchone() != (0,):
                raise P638All10RankingSchemaMigrationError(
            ...
            try:
                ...
                if version is None:
                    ...
                    connection.execute(
                        """
                        INSERT INTO p638_all10_schema_migrations
                ...
                if version != CURRENT_SCHEMA_VERSION:
                    raise P638All10RankingSchemaMigrationError(
                        f"P638 all-10 ranking schema migration did not reach version "
                        f"{CURRENT_SCHEMA_VERSION}"
                    )
                if connection.execute("PRAGMA foreign_key_check").fetchall():
                    raise P638All10RankingSchemaMigrationError(
                        "P638 all-10 ranking schema migration left foreign-key violations"
                    )
...
def verify_schema_read_only(database: Path) -> bool:
    ...
    if not initialized:
        raise P638All10RankingSchemaMigrationError("database exists without a schema migration")
    return True
...
def open_database(database: Path, *, read_only: bool = False) -> Generator[sqlite3.Connection]:
    ...
    if not paths.database.exists():
        raise P638All10RankingSchemaMigrationError(
            "P638 all-10 ranking database does not exist"
        )
...
@contextmanager
def _raw_connection(
    paths: P638All10RankingDatabasePaths, *, read_only: bool = False
    ...
    try:
        ...
        raise P638All10RankingSchemaMigrationError(
            "cannot open the P638 all-10 ranking database safely"
        ) from exc
    try:
        try:
            ...
            raise P638All10RankingSchemaMigrationError("cannot configure SQLite safely") from exc
        yield connection
    finally:
        connection.close()
...
def _verify_migration_state(connection: sqlite3.Connection) -> int | None:
    ...
    if "p638_all10_schema_migrations" not in table_names:
        if table_names:
            raise P638All10RankingSchemaMigrationError(
    ...
    rows = connection.execute(
        "SELECT version, name, checksum FROM p638_all10_schema_migrations ORDER BY version"
    ).fetchall()
    try:
        versions = [int(row[0]) for row in rows]
    except (TypeError, ValueError) as exc:
        raise P638All10RankingSchemaMigrationError("migration versions are invalid") from exc
    if any(version > CURRENT_SCHEMA_VERSION for version in versions):
        raise P638All10RankingSchemaMigrationError(
            "database schema is newer than this LottoLab build"
        )
    if versions != [1]:
        raise P638All10RankingSchemaMigrationError("migration history is incomplete")
    _, name, checksum = rows[0]
    if name != MIGRATION_NAME or checksum != MIGRATION_CHECKSUM:
        raise P638All10RankingSchemaChecksumError("migration checksum does not match")
...
def _verify_schema_semantics(connection: sqlite3.Connection, table_names: set[str]) -> None:
    if table_names != set(TABLE_NAMES):
        raise P638All10RankingSchemaMigrationError("database tables do not match the schema")
    ...
    for row in schema_rows:
        ...
        if expected_sql is None or not isinstance(actual_sql, str):
            raise P638All10RankingSchemaMigrationError(f"unexpected database schema object: {name}")
    ...
    if seen_names != set(_EXPECTED_SCHEMA_SQL_BY_NAME):
        raise P638All10RankingSchemaMigrationError("database schema objects do not match")
...
__all__ = [
    ...
    "TABLE_NAMES",
    "P638All10RankingDatabasePaths",
    "P638All10RankingSchemaChecksumError",
    "P638All10RankingSchemaError",
    "P638All10RankingSchemaMigrationError",
    "initialize_schema",
    "open_database",
    "resolve_p638_all10_ranking_database_paths",
