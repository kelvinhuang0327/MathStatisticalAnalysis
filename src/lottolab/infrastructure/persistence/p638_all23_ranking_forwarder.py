"""Builds the P638 all-23 official-prize historical ranking projection.
...
must never be mutated) and from ``p638_all10_ranking_forwarder.py``'s
all-10 executable-strategy contract (Wave 1 only, also never mutated by
this module): this module runs a fresh causal replay of all 23 currently
executable POWER_LOTTO strategies -- Wave 1's 10 plus Wave 2's 13 -- against
the reconciled draw authority, evaluates every complete ticket under the
official POWER_LOTTO prize-tier table (not the 8-strategy contract's
``combined_zone1_4plus_zone2_hit_count`` heuristic), ranks all 23 by
historical winning-target rate, and writes the
``P638_HISTORICAL_RESULTS_ALL23_PRIZE_RANKING_V1`` projection.
...
ALL23_STRATEGIES: tuple[P638StrategySpec, ...] = WAVE1_STRATEGIES + WAVE2_STRATEGIES
ALL23_SELECTED_STRATEGY_IDS: tuple[str, ...] = tuple(
...
class P638All23RankingBuildError(RuntimeError):
    """The P638 all-23 replay/ranking build failed reconciliation."""


@dataclass(frozen=True, slots=True)
class P638All23RankingBuildResult:
    run_id: str
...
def _load_draws_from_draw_db(draw_db: Path) -> tuple[tuple[PowerLottoDrawRecord, ...], str]:
    ...
    try:
        ...
        if migration is None:
            raise P638All23RankingBuildError(
        ...
        if not rows:
            raise P638All23RankingBuildError("draw database has no POWER_LOTTO draws")
...
def _compute_ranking(
    *,
    ...
    return P638RankingRecord(
        run_id=run_id,
        rank=0,  # assigned by the caller after sorting all 23
    ...
    with open_database(output_db) as connection:
        ...
        try:
            existing = connection.execute(
                "SELECT 1 FROM p638_all23_run WHERE run_id = ?", (run_id,)
            ).fetchone()
            ...
            for spec in ALL23_STRATEGIES:
                for target in targets_by_strategy.get(spec.strategy_id, ()):
                    ...
                    target_id = f"p638-all23-target-{digest}"
            ...
            for spec in ALL23_STRATEGIES:
                ...
                for ticket in tickets_by_strategy.get(spec.strategy_id, ()):
                    ...
                    if target_id is None:
                        raise P638All23RankingBuildError(
                            "P638 all-23 ticket has no target row"
                        )
                    ticket_id = (
                        f"p638-all23-ticket-"
                ...
                for target_draw_number, target_id in (
                    ...
                    connection.execute(
                        "UPDATE p638_all23_target SET target_is_winner = ? WHERE id = ?",
            ...
            for record in rankings:
                connection.execute(
                    """
                    INSERT INTO p638_all23_ranking (
            ...
            if connection.execute("PRAGMA foreign_key_check").fetchall():
                raise P638All23RankingBuildError(
                    "P638 all-23 ranking projection left foreign-key violations"
                )
...
def build_p638_all23_ranking(
    *,
    draw_db: Path,
    runtime_root: Path,
    output_db: Path,
    replay_db_path: Path | None = None,
) -> P638All23RankingBuildResult:
    """Run the all-23 causal replay and build the official-prize ranking projection."""

    draws, draw_db_sha256 = _load_draws_from_draw_db(draw_db)
    replay_result = run_replay(
        draws=draws,
        strategy_objects=ALL23_STRATEGIES,
        runtime_root=runtime_root,
        db_path=replay_db_path,
        selected_strategy_ids=ALL23_SELECTED_STRATEGY_IDS,
    )
    if replay_result.failed_target_count != 0:
        raise P638All23RankingBuildError(
            "P638 all-23 replay produced failed targets: "
            f"{replay_result.failed_target_count}"
        )
    if replay_result.selected_count != len(ALL23_STRATEGIES):
        raise P638All23RankingBuildError(
            "P638 all-23 replay did not select all 23 executable strategies"
        )
    ...
    rankings = tuple(
        replace(record, rank=index) for index, record in enumerate(ordered, start=1)
    )
    ...
    return P638All23RankingBuildResult(
        run_id=replay_result.run_id,
        contract_version=CONTRACT_VERSION,
        source_replay_db_sha256=replay_db_sha256,
        source_draw_db_sha256=draw_db_sha256,
        draw_count=len(draws),
        strategy_count=len(ALL23_STRATEGIES),
