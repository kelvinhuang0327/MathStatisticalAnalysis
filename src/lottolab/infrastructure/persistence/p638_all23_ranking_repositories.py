"""Read-only SQLite queries for the P638 all-23 official-prize ranking projection.

Distinct from ``p638_historical_repositories.py`` (the frozen 8-strategy
P638 Historical Results V2 database) and from
``p638_all10_ranking_repositories.py`` (the separate all-10
executable-strategy database): this module reads the separate all-23
executable-strategy ranking database built by
``p638_all23_ranking_forwarder.py``.
"""
...
class SQLiteP638All23RankingQueryRepository:
    """Explicit-path, read-only implementation of the P638 all-23 ranking query port.

    The all-23 ranking projection is built as one bounded analytical run
    ...
    def _resolve_run_id(connection: sqlite3.Connection, run_id: str) -> str | None:
        if run_id == LATEST_RUN_TOKEN:
            row = connection.execute(
                "SELECT run_id FROM p638_all23_run ORDER BY completed_at DESC, run_id DESC LIMIT 1"
            ).fetchone()
...
def _verify_available(database: Path) -> bool:
    ...
    except (P638All23RankingSchemaError, sqlite3.Error) as exc:
        raise P638HistoricalResultsUnavailableError(
            "P638 all-23 ranking storage failed schema verification"
        ) from exc
...
def _read_only_connection(database: Path) -> Generator[sqlite3.Connection]:
    try:
        ...
        raise P638HistoricalResultsUnavailableError(
            "P638 all-23 ranking storage is unavailable"
        ) from exc
...
__all__ = ["LATEST_RUN_TOKEN", "SQLiteP638All23RankingQueryRepository"]
