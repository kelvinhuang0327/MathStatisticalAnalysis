...
V2_MIGRATION_NAME = "allow_p638_all23_source_native_portfolio_closure"
...
V2_MIGRATION_STATEMENTS = (
    "ALTER TABLE p638_all23_target RENAME TO p638_all23_target_v1",
    _V2_TARGET_TABLE_STATEMENT,
    """
    ...
    SELECT
        ...
        target_draw_date, cutoff_draw_number, history_length,
        expected_ticket_count, status, target_is_winner
    FROM p638_all23_target_v1
    """,
    "DROP TABLE p638_all23_target_v1",
    """
    CREATE INDEX idx_p638_all23_target_query
    ON p638_all23_target (run_id, strategy_id, target_draw_date, target_draw_number, status)
    """,
...
def initialize_schema(database: Path) -> None:
    ...
    with _raw_connection(paths) as connection:
        try:
            ...
            if connection.execute("PRAGMA legacy_alter_table").fetchone() != (1,):
                raise P638All23RankingSchemaMigrationError(
                    "cannot preserve child foreign keys during atomic table rebuild"
                )
            ...
            try:
                ...
                if version == 1:
                    ...
                    connection.execute(
                        ...
                        VALUES (?, ?, ?, ?)
                        """,
                        (2, V2_MIGRATION_NAME, V2_MIGRATION_CHECKSUM, _utc_now()),
                    )
                    version = _verify_migration_state(connection)
            ...
            if connection.execute("PRAGMA legacy_alter_table").fetchone() != (0,):
                raise P638All23RankingSchemaMigrationError(
                    "cannot restore SQLite alter-table behavior"
                )
...
def verify_schema_read_only(database: Path) -> bool:
    ...
    if initialized != CURRENT_SCHEMA_VERSION:
...
def open_database(database: Path, *, read_only: bool = False) -> Generator[sqlite3.Connection]:
    ...
    with _raw_connection(paths, read_only=read_only) as connection:
        if _verify_migration_state(connection) != CURRENT_SCHEMA_VERSION:
            raise P638All23RankingSchemaMigrationError(
...
def _verify_migration_state(connection: sqlite3.Connection) -> int | None:
    ...
    expected_migrations = (
        ...
        (2, V2_MIGRATION_NAME, V2_MIGRATION_CHECKSUM),
    )
    for row, expected in zip(rows, expected_migrations[: len(rows)], strict=True):
        version_value, name, checksum = row
        if (int(version_value), name, checksum) != expected:
            raise P638All23RankingSchemaChecksumError(
                f"migration checksum does not match version {version_value}"
            )
...
def _verify_schema_semantics(
    ...
    *,
    ...
    expected_schema = (
        _V1_EXPECTED_SCHEMA_SQL_BY_NAME if version == 1 else _EXPECTED_SCHEMA_SQL_BY_NAME
    )
