"""Read-only SQLite queries for the P638 current-universe official-prize ranking projection.

Distinct from ``p638_historical_repositories.py`` (the frozen 8-strategy
P638 Historical Results V2 database), from ``p638_all10_ranking_repositories.py``,
and from ``p638_all23_ranking_repositories.py``: this module reads the
current-universe ranking database built by ``p638_current_ranking_forwarder.py``,
whose strategy count grows across waves rather than being fixed.
"""
...
class SQLiteP638CurrentRankingQueryRepository:
    """Explicit-path, read-only implementation of the P638 current ranking query port.

    The current-universe ranking projection is built as one bounded
    analytical run rather than a continuously growing history, so callers
    may pass the reserved ``"latest"`` token instead of an exact ``run_id``
    to resolve the most recently completed run without a separate discovery
    endpoint.
    """
    ...
    def list_rankings(self, run_id: str) -> P638CurrentRankingPage | None:
        ...
        with _read_only_connection(self._database) as connection:
            ...
            fingerprint_row = connection.execute(
                "SELECT strategy_set_fingerprint FROM p638_current_run WHERE run_id = ?",
                (run_id,),
            ).fetchone()
            ...
            rows = connection.execute(
                ...
                FROM p638_current_ranking
                WHERE run_id = ?
                ORDER BY rank ASC
                """,
                (run_id,),
            ).fetchall()
        return P638CurrentRankingPage(
            run_id=run_id,
            strategy_set_fingerprint=str(fingerprint_row[0]),
            items=tuple(_row_to_ranking(row) for row in rows),
        )
    ...
    def _resolve_run_id(connection: sqlite3.Connection, run_id: str) -> str | None:
        if run_id == LATEST_RUN_TOKEN:
            row = connection.execute(
                "SELECT run_id FROM p638_current_run "
...
def _verify_available(database: Path) -> bool:
    ...
    except (P638CurrentRankingSchemaError, sqlite3.Error) as exc:
        raise P638HistoricalResultsUnavailableError(
            "P638 current-universe ranking storage failed schema verification"
        ) from exc
...
__all__ = ["LATEST_RUN_TOKEN", "SQLiteP638CurrentRankingQueryRepository"]
