"""CLI for the wave-63 advanced-method BIG_LOTTO batch."""
...
class LegacyAdvancedMethodsNativeWave63BatchCliError(RuntimeError):
...
def materialize_legacy_advanced_methods_native_wave63_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_SOURCE_NATIVE_WAVE63_USER_SEED,
) -> str:
    """Write one canonical wave-63 input atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacyAdvancedMethodsNativeWave63BatchCliError(
    ...
    except LegacyAdvancedMethodsNativeWave63BatchImportError as exc:
        raise LegacyAdvancedMethodsNativeWave63BatchCliError(
            str(exc)
        ) from exc
...
def materialize_legacy_advanced_methods_native_wave63_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for the wave-63 method."""

    try:
        summary = (
            materialize_legacy_advanced_methods_native_wave63_batch_file(
                database=database,
                expected_database_sha256=expected_database_sha256,
    ...
    except LegacyAdvancedMethodsNativeWave63BatchCliError as exc:
...
__all__ = [
    "LegacyAdvancedMethodsNativeWave63BatchCliError",
