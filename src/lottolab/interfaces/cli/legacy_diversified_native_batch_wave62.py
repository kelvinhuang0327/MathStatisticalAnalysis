"""CLI for the wave-62 frozen diversified BIG_LOTTO batch."""
...
class LegacyDiversifiedNativeWave62BatchCliError(RuntimeError):
...
def materialize_legacy_diversified_native_wave62_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_SOURCE_NATIVE_WAVE62_USER_SEED,
) -> str:
    """Write one canonical wave-62 input atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacyDiversifiedNativeWave62BatchCliError(
            f"refusing to overwrite existing output: {output_file}"
        )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    try:
        document = materialize_legacy_diversified_native_wave62_batch(
            database=database,
            expected_database_sha256=expected_database_sha256,
            user_seed=user_seed,
        )
    except LegacyDiversifiedNativeWave62BatchImportError as exc:
        raise LegacyDiversifiedNativeWave62BatchCliError(
            str(exc)
        ) from exc
...
def materialize_legacy_diversified_native_wave62_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for wave-62 diversified methods."""

    try:
        summary = (
            materialize_legacy_diversified_native_wave62_batch_file(
                database=database,
                expected_database_sha256=expected_database_sha256,
    ...
    except LegacyDiversifiedNativeWave62BatchCliError as exc:
...
__all__ = [
    "LegacyDiversifiedNativeWave62BatchCliError",
