"""CLI for the wave-45 frozen FFT BIG_LOTTO batch."""
...
class LegacyFftNativeWave45BatchCliError(RuntimeError):
...
def materialize_legacy_fft_native_wave45_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_SOURCE_NATIVE_WAVE45_USER_SEED,
) -> str:
    """Write one canonical wave-45 input atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacyFftNativeWave45BatchCliError(
            f"refusing to overwrite existing output: {output_file}"
        )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    try:
        document = materialize_legacy_fft_native_wave45_batch(
            database=database,
            expected_database_sha256=expected_database_sha256,
            user_seed=user_seed,
        )
    except LegacyFftNativeWave45BatchImportError as exc:
        raise LegacyFftNativeWave45BatchCliError(str(exc)) from exc
...
def materialize_legacy_fft_native_wave45_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for the wave-45 FFT methods."""

    try:
        summary = materialize_legacy_fft_native_wave45_batch_file(
            database=database,
            expected_database_sha256=expected_database_sha256,
            output_file=output_file,
            user_seed=user_seed,
        )
    except LegacyFftNativeWave45BatchCliError as exc:
...
__all__ = [
    "LegacyFftNativeWave45BatchCliError",
    "materialize_legacy_fft_native_wave45_batch_command",
    "materialize_legacy_fft_native_wave45_batch_file",
]
