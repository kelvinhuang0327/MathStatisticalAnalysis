"""CLI for the four-method frozen history-native batch."""
...
class LegacyHistoryNativeBatchCliError(RuntimeError):
...
def materialize_legacy_history_native_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_HISTORY_NATIVE_USER_SEED,
) -> str:
    """Write one canonical artifact atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacyHistoryNativeBatchCliError(
    ...
    except LegacyHistoryNativeBatchImportError as exc:
        raise LegacyHistoryNativeBatchCliError(str(exc)) from exc
...
def materialize_legacy_history_native_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for four frozen history-native methods."""

    try:
        summary = materialize_legacy_history_native_batch_file(
            database=database,
            expected_database_sha256=expected_database_sha256,
            output_file=output_file,
            user_seed=user_seed,
        )
    except LegacyHistoryNativeBatchCliError as exc:
...
__all__ = [
    "LegacyHistoryNativeBatchCliError",
    "materialize_legacy_history_native_batch_command",
    "materialize_legacy_history_native_batch_file",
]
