"""CLI for the second four-method frozen history-native batch."""
...
class LegacyHistoryNativeWave2BatchCliError(RuntimeError):
...
def materialize_legacy_history_native_wave2_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_HISTORY_NATIVE_WAVE2_USER_SEED,
) -> str:
    """Write one canonical wave-2 artifact atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacyHistoryNativeWave2BatchCliError(
    ...
    except LegacyHistoryNativeWave2BatchImportError as exc:
        raise LegacyHistoryNativeWave2BatchCliError(str(exc)) from exc
...
def materialize_legacy_history_native_wave2_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for the second history-native batch."""

    try:
        summary = materialize_legacy_history_native_wave2_batch_file(
            database=database,
            expected_database_sha256=expected_database_sha256,
            output_file=output_file,
            user_seed=user_seed,
        )
    except LegacyHistoryNativeWave2BatchCliError as exc:
...
__all__ = [
    "LegacyHistoryNativeWave2BatchCliError",
    "materialize_legacy_history_native_wave2_batch_command",
    "materialize_legacy_history_native_wave2_batch_file",
]
