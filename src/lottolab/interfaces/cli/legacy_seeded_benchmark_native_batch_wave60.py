"""CLI for the wave-60 seeded benchmark selector batch."""
...
class LegacySeededBenchmarkNativeWave60BatchCliError(RuntimeError):
...
def materialize_legacy_seeded_benchmark_native_wave60_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_SOURCE_NATIVE_WAVE60_USER_SEED,
) -> str:
    """Write one canonical wave-60 input atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacySeededBenchmarkNativeWave60BatchCliError(
    ...
    except LegacySeededBenchmarkNativeWave60BatchImportError as exc:
        raise LegacySeededBenchmarkNativeWave60BatchCliError(
            str(exc)
        ) from exc
...
def materialize_legacy_seeded_benchmark_native_wave60_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal input for all three wave-60 frozen source methods."""

    try:
        summary = (
            materialize_legacy_seeded_benchmark_native_wave60_batch_file(
                database=database,
                expected_database_sha256=expected_database_sha256,
    ...
    except LegacySeededBenchmarkNativeWave60BatchCliError as exc:
...
__all__ = [
    "LegacySeededBenchmarkNativeWave60BatchCliError",
