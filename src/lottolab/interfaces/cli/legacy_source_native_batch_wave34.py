"""CLI for the thirty-fourth frozen source-native BIG_LOTTO batch."""
...
class LegacySourceNativeWave34BatchCliError(RuntimeError):
...
def materialize_legacy_source_native_wave34_batch_file(
    *,
    database: Path,
    expected_database_sha256: str,
    output_file: Path,
    user_seed: str = DEFAULT_SOURCE_NATIVE_WAVE34_USER_SEED,
) -> str:
    """Write one canonical wave-34 artifact atomically and refuse overwrite."""

    if output_file.exists():
        raise LegacySourceNativeWave34BatchCliError(
    ...
    except LegacySourceNativeWave34BatchImportError as exc:
        raise LegacySourceNativeWave34BatchCliError(str(exc)) from exc
...
def materialize_legacy_source_native_wave34_batch_command(
    database: Annotated[
        Path,
        typer.Option(
            "--database",
    ...
    """Create causal evaluator input for the wave-34 optimizer."""

    try:
        summary = materialize_legacy_source_native_wave34_batch_file(
            database=database,
            expected_database_sha256=expected_database_sha256,
            output_file=output_file,
            user_seed=user_seed,
        )
    except LegacySourceNativeWave34BatchCliError as exc:
...
__all__ = [
    "LegacySourceNativeWave34BatchCliError",
    "materialize_legacy_source_native_wave34_batch_command",
    "materialize_legacy_source_native_wave34_batch_file",
]
