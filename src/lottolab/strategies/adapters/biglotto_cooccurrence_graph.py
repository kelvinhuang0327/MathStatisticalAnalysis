...
def _ticket(numbers: list[int]) -> tuple[int, ...]:
    ...
    if (
        ...
        raise InvalidOutput(
            f"{_STRATEGY_ID}: Cooccurrence Graph ticket is not a legal 6-of-49 set"
        )
    return values
...
def _cooccurrence_graph_tickets(
    ...
    while len(predictions) < _MAXIMUM_NATIVE_TICKET_COUNT:
        if not page_rank:
            candidate = _ticket(
                python_rng.sample(range(_MIN_NUMBER, _MAX_NUMBER + 1), _PICK_COUNT)
            )
        else:
            candidates = [
                number
...
class BigLottoCooccurrenceGraphAdapter(PortfolioBetAdapter):
    ...
    def __init__(self, *, rng_seed: int = _DEFAULT_TARGET_SEED) -> None:
        if (
            type(rng_seed) is not int
            or not 0 <= rng_seed <= _MAX_RANDOM_STATE_SEED
        ):
            raise InvalidOutput(
                f"{self.strategy_id}: rng_seed must be an integer in "
                f"[0..{_MAX_RANDOM_STATE_SEED}]"
            )
