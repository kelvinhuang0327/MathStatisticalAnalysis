...
def _hot_stop_statistics(
    ...
    frequency = Counter(number for row in recent_frequency for number in row.numbers)
    appeared_in_recent = {number for row in history[-_RECENT_GAP_WINDOW:] for number in row.numbers}
    gaps: dict[int, int] = {}
    for number in range(_MIN_NUMBER, _MAX_NUMBER + 1):
        ...
        gaps[number] = gap
    frequencies = {
        number: frequency.get(number, 0) for number in range(_MIN_NUMBER, _MAX_NUMBER + 1)
    }
...
def _hot_stop_ticket(
    *,
    ...
    candidates = [
        ...
        for number in range(_MIN_NUMBER, _MAX_NUMBER + 1)
        if frequencies[number] >= frequency_threshold and gaps[number] >= gap_threshold
    ]
