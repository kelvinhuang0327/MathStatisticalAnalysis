...
def _tickets_from_rank(
    ...
    if len(fourier_rank) != _MAX_NUMBER + 1 or set(fourier_rank) != set(range(_MAX_NUMBER + 1)):
    ...
    hot_leftover = [number for number in range(_MIN_NUMBER, _MAX_NUMBER + 1) if number not in used]
...
def _validated_orthogonal_portfolio(
    ...
    if len(tickets) != _NATIVE_TICKET_COUNT:
        raise InvalidOutput(f"{_STRATEGY_ID}: expected {_NATIVE_TICKET_COUNT} positional tickets")
    seen: set[int] = set()
    for position, ticket in enumerate(tickets, start=1):
        ...
        if not seen.isdisjoint(ticket):
            raise InvalidOutput(f"{_STRATEGY_ID}: donor orthogonality failed at ticket {position}")
        seen.update(ticket)
