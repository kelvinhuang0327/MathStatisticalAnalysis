    ...
    for method_name, weight in _ENGINE_WEIGHTS:
        ...
        for index, number in enumerate(result):
            if number not in exclude_range:
                candidates[number] = candidates.get(number, 0.0) + (20 - index) * weight
    ranked = sorted(
        candidates,
...
def _ticket(numbers: tuple[int, ...]) -> tuple[int, ...]:
    ...
    if (
        ...
        raise InvalidOutput(f"{_STRATEGY_ID}: frozen donor emitted fewer than six legal candidates")
    return values
...
def _radical_gap_tickets(
    ...
    first, second = (_ticket(pool[:_PICK_COUNT]) for pool in candidate_pools)
    return first, second
