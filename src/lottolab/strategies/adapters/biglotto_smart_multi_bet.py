...
def _ticket(numbers: list[int]) -> tuple[int, ...]:
    ...
    if (
        ...
        or any(
            type(number) is not int or not _MIN_NUMBER <= number <= _MAX_NUMBER
            for number in values
        )
    ):
        raise InvalidOutput(
            f"{_STRATEGY_ID}: Smart Multi-Bet ticket is not a legal 6-of-49 set"
        )
    return values
...
def _build_candidate_pool(
    ...
    recent_active = [
        number for number, count in recent_frequency.items() if count >= 2
    ]
    ...
    for number in all_numbers:
        ...
        for index, draw in enumerate(recent_first):
            if number in draw.numbers:
                ...
                break
        appearances = [
            index for index, draw in enumerate(recent_first) if number in draw.numbers
        ]
...
def _zone_coverage(
    ...
    for zone in zones:
        candidates = [
            number for number in zone if number not in used and number not in result
        ]
...
def _constrained(
    ...
    for _ in range(200):
        ...
        if len(candidates) < _PICK_COUNT:
            remaining = [
                number for number in pool["all"] if number not in candidates
            ]
