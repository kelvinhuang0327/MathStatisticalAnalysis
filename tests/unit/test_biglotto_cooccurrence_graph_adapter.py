...
def test_graph_nodes_edges_weights_rankings_and_communities_match_retained(
    ...
    assert {
        number for edge in target.edges for number in edge
...
def test_duplicate_stop_preserves_one_ticket_native_closure() -> None:
    ...
    assert BigLottoCooccurrenceGraphAdapter(rng_seed=42).get_bets(
        history,
        LotteryType.BIG_LOTTO,
    ) == expected
...
def test_replay_count_seam_resolves_the_same_seeded_best_effort_count() -> None:
    ...
    assert BigLottoCooccurrenceGraphAdapter(rng_seed=42).expected_native_ticket_count(
        strategy,
        replay_history,
        target,
    ) == 1
...
def test_same_seed_is_deterministic_isolated_and_window_causal() -> None:
    ...
    changed_outside_window = tuple(
        CausalDrawRow(row.draw, row.date, (44, 45, 46, 47, 48, 49))
        if index < 30
        else row
    ...
    assert adapter.with_seed(104729).get_bets(
