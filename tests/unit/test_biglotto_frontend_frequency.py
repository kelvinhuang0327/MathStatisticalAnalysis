"""Executable old/new parity for the legacy frontend Frequency donor."""
...
from lottolab.strategies.adapters import BigLottoFrontendFrequencyAdapter
...
def _stride_row(index: int) -> CausalDrawRow:
    ...
    return CausalDrawRow(
        draw=f"frontend-frequency-{index}",
...
def test_matches_executed_donor_golden(length: int) -> None:
    assert BigLottoFrontendFrequencyAdapter().get_one_bet(
...
def test_minimum_history_and_donor_fallback_are_explicit() -> None:
    adapter = BigLottoFrontendFrequencyAdapter()
...
def test_catalog_and_registry_add_exactly_one_online_identity() -> None:
    ...
    assert len(catalog) == 125
    assert descriptor.strategy_id == BigLottoFrontendFrequencyAdapter.strategy_id
    assert descriptor.strategy_name == BigLottoFrontendFrequencyAdapter.strategy_name
    assert descriptor.version == BigLottoFrontendFrequencyAdapter.strategy_version
    ...
    assert (
        ...
        "2e3e8febb5f1eea29fe2105d955b65b80188f0259d78f23b4ca80a0468a128e2"
        in descriptor.provenance
    )
    assert "legacy_symbol:FrequencyStrategy.predict" in descriptor.provenance
    assert "legacy_runtime:PredictionEngine.strategies.frequency" in descriptor.provenance
    assert (
...
def test_production_generation_never_opens_a_database(
    ...
    def _forbidden_connect(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("Frontend Frequency Strategy must not open a database")
