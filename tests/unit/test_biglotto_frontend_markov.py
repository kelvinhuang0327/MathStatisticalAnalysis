"""Executable old/new parity for the legacy frontend Markov donor.

The expected tickets were produced by executing the actual donor entry point
``MarkovStrategy.predict`` with Node 20.19.5 against the same oldest-first
histories reversed into the donor's newest-first input order. The donor was
executed from ``LotteryNewMeraged/src/engine/strategies/MarkovStrategy.js``
...
from lottolab.strategies.adapters import BigLottoFrontendMarkovAdapter
...
def test_matches_executed_donor_golden(length: int) -> None:
    assert BigLottoFrontendMarkovAdapter().get_one_bet(
...
def test_minimum_history_and_donor_fallback_are_explicit() -> None:
    adapter = BigLottoFrontendMarkovAdapter()
...
def test_ranking_ties_preserve_ascending_integer_key_order() -> None:
    history = (
        ...
        CausalDrawRow("tie-1", "tie-1", (1, 2, 3, 4, 5, 7)),
        CausalDrawRow("tie-2", "tie-2", (1, 2, 3, 4, 5, 8)),
    )
    assert BigLottoFrontendMarkovAdapter().get_one_bet(history, LotteryType.BIG_LOTTO) == (
        (1, 2, 3, 4, 5, 7),
        None,
    )
...
def test_edge_number_boundaries_and_history_order_match_donor() -> None:
    history = (
        ...
        CausalDrawRow("edge-2", "edge-2", (1, 44, 45, 46, 47, 48)),
        CausalDrawRow("edge-3", "edge-3", (44, 45, 46, 47, 48, 49)),
    )
    assert BigLottoFrontendMarkovAdapter().get_one_bet(history, LotteryType.BIG_LOTTO) == (
        (1, 44, 45, 46, 47, 48),
        None,
...
def test_catalog_and_registry_add_exactly_one_online_identity() -> None:
    ...
    assert ExecutableRegistry(catalog).load_adapter(STRATEGY_ID) is BigLottoFrontendMarkovAdapter
