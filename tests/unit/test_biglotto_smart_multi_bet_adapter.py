...
def test_production_catalog_preserves_smart_multi_bet_append_position() -> None:
    ...
    assert len(strategy_ids) == 79
    assert strategy_ids[-4] == STRATEGY_ID
    assert strategy_ids[:-4].count(STRATEGY_ID) == 0
    assert strategy_ids[-7:-4] == (
