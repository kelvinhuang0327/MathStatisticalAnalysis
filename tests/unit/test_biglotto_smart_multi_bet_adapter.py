...
def _history(count: int) -> tuple[CausalDrawRow, ...]:
    ...
    for index in range(count):
        values = sorted(
            ((index * 7 + offset * 5) % 49) + 1 for offset in range(6)
        )
        rows.append(
            CausalDrawRow(
                draw=str(index + 1),
...
def test_candidate_pool_matches_preserved_donor_components(
    ...
    assert _build_candidate_pool(history) == _build_smart_multi_pool(
...
def test_catalog_registry_and_generation_dispatch_preserve_portfolio() -> None:
    ...
    assert "runtime_boundary:CALLER_SUPPLIED_CAUSAL_HISTORY_NO_DB" in (
...
def test_production_catalog_preserves_smart_multi_bet_append_position() -> None:
    ...
    assert strategy_ids[-5] == STRATEGY_ID
    assert strategy_ids[:-5].count(STRATEGY_ID) == 0
    assert strategy_ids[-8:-5] == (
