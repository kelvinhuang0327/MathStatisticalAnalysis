...
def test_production_catalog_preserves_smart_multi_bet_append_position() -> None:
    ...
    assert strategy_ids[-8] == STRATEGY_ID
    assert strategy_ids[:-8].count(STRATEGY_ID) == 0
    assert strategy_ids[-11:-8] == (
    ...
    assert strategy_ids[-7:-5] == (
        "legacy_biglotto__anti_consensus_strategy__a454ddd26cef",
        "legacy_biglotto__cooccurrence_graph__25fa2e473092",
    )
    assert strategy_ids[-5] == "legacy_biglotto__backtest_radical_strategy__e54cc0812bc6"
    assert strategy_ids[-4] == "legacy_biglotto__power_fourier_rhythm__cb75e72e4c94"
    assert strategy_ids[-3] == ("legacy_biglotto__backtest_big_lotto_orthogonal_5bet__c4dff46c5a5e")
    assert strategy_ids[-2] == "legacy_biglotto__predict_biglotto_quad_strike__e202e664208f"
    assert strategy_ids[-1] == "legacy_biglotto__frontend_markov_strategy__2fc1cafea55c"
