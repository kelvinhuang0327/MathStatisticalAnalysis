"""Causal batch and CLI contracts for wave 58."""
...
import lottolab.infrastructure.legacy_dual_seeded_native_batch_import_wave58 as batch_module
...
def test_wave58_batch_preserves_per_method_causal_coverage(
    monkeypatch: Any,
) -> None:
    def fake_load(
        *,
    ...
    document = (
        batch_module.materialize_legacy_dual_seeded_native_wave58_batch(
    ...
    assert successful[0]["native_ticket_count"] == 3
    ...
    assert provenance["native_ticket_count"] == {
...
def test_wave58_cli_is_registered() -> None:
    result = runner.invoke(app, ["--help"])

    assert result.exit_code == 0
    assert (
        "materialize-biglotto-dual-seeded-native-wave58-batch"
        in result.stdout
    )
