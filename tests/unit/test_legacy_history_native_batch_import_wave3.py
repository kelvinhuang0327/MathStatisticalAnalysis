"""Causal batch tests for the third history-native wave."""
...
def test_wave3_batch_preserves_combination_and_closed_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert len(successful) == 27
    assert len(closed) == 3
    assert all(len(row["ordered_portfolio"]) == 20 for row in successful)
    assert all("combination_count" not in row for row in closed)
    assert all(
    ...
    assert by_method[CORE_SATELLITE_METHOD_ID]["native_ticket_count"] == 12
    assert by_method[CORE_SATELLITE_METHOD_ID]["combination_count"] == 4
    assert by_method[NEGATIVE_SELECTION_METHOD_ID]["native_ticket_count"] == 8
    assert by_method[NEGATIVE_SELECTION_METHOD_ID]["combination_count"] == 2
    assert by_method[QUANTUM_RANDOM_METHOD_ID]["native_ticket_count"] == 8
    assert "combination_count" not in by_method[QUANTUM_RANDOM_METHOD_ID]
    assert provenance["database_sha256_before"] == database_sha256
    assert provenance["database_sha256_after"] == database_sha256


def test_wave3_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    ...
    assert report["target_draw_count"] == 10
...
def test_wave3_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
