"""Causal batch tests for the fifth history-native wave."""
...
def test_wave5_batch_preserves_native_and_closed_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert by_method[MODERATE_SELECTION_METHOD_ID][
        "native_ticket_count"
    ] == 3
    assert by_method[MODERATE_SELECTION_METHOD_ID][
        "combination_count"
    ] == 2
    assert by_method[DIVERSIFIED_2BET_METHOD_ID][
        "native_ticket_count"
    ] == 8
    assert by_method[DIVERSIFIED_2BET_METHOD_ID][
        "combination_count"
    ] == 5
    assert by_method[ECHO_2BET_METHOD_ID][
        "native_ticket_count"
    ] == 2
    assert "combination_count" not in by_method[ECHO_2BET_METHOD_ID]
    assert all(
...
def test_wave5_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave5_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
