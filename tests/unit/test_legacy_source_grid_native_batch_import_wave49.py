"""Causal materialization contracts for wave 49."""
...
import lottolab.application.legacy_source_grid_native_portfolios_wave49 as portfolio_module
import lottolab.infrastructure.legacy_source_grid_native_batch_import_wave49 as batch_module
...
def _fake_history() -> PinnedBigLottoHistory:
    ledger = portfolio_module.load_legacy_source_grid_native_wave49_ledger_for_verification()
    base = date(2000, 1, 1)
    draws = [
        PinnedBigLottoDraw(
...
def test_wave49_batch_preserves_minimums_native_positions_and_ordered20(
    monkeypatch: Any,
) -> None:
    ledger = portfolio_module.load_legacy_source_grid_native_wave49_ledger_for_verification()
    ...
    document = batch_module.materialize_legacy_source_grid_native_wave49_batch(
    ...
    assert len(executions) == 1950
    assert len(successful) == 152
    assert len(closed) == 1798
    assert all(
        row["candidate_k"] == 49
    ...
    assert provenance["execution_status_counts"] == {
        "CLOSED_INSUFFICIENT_HISTORY": 1798,
        "OK": 152,
    }
    assert (
        cast(dict[str, int], provenance["native_ticket_count"])[AUTO_DISCOVERY_METHOD_ID]
        == 54
    )
    assert (
...
def test_wave49_cli_is_registered() -> None:
    result = runner.invoke(app, ["--help"])

    assert result.exit_code == 0
    assert "materialize-biglotto-source-grid-native-wave49-batch" in result.stdout
