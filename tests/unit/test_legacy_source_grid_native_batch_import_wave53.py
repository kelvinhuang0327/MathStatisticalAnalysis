"""Causal materialization contracts for wave 53."""
...
import lottolab.application.legacy_source_grid_native_portfolios_wave53 as portfolio_module
import lottolab.infrastructure.legacy_source_grid_native_batch_import_wave53 as batch_module
...
def _fake_history() -> PinnedBigLottoHistory:
    ledger = portfolio_module.load_legacy_source_grid_native_wave53_ledger_for_verification()
    base = date(2000, 1, 1)
    draws = [
        PinnedBigLottoDraw(
...
def test_wave53_batch_preserves_minimums_native_positions_and_ordered20(
    monkeypatch: Any,
) -> None:
    ledger = portfolio_module.load_legacy_source_grid_native_wave53_ledger_for_verification()
    ...
    document = batch_module.materialize_legacy_source_grid_native_wave53_batch(
    ...
    assert len(executions) == 1002
    assert len(successful) == 302
    ...
    assert provenance["execution_status_counts"] == {
        "CLOSED_INSUFFICIENT_HISTORY": 700,
        "OK": 302,
    }
    assert (
        cast(dict[str, int], provenance["native_ticket_count"])[ANALYSIS_METHOD_ID]
        == 14
    )
    assert (
...
def test_wave53_cli_is_registered() -> None:
    result = runner.invoke(app, ["--help"])

    assert result.exit_code == 0
    assert "materialize-biglotto-source-grid-native-wave53-batch" in result.stdout
