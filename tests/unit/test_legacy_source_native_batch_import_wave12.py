"""Causal batch tests for the twelfth source-native wave."""
...
def test_wave12_batch_preserves_grid_and_closed_semantics(
    tmp_path: Path,
) -> None:
    ...
    first = successful[0]
    assert first["combination_count"] == 180
    assert first["native_ticket_count"] == 360
    native = cast(dict[str, Any], first["native_generation"])
    assert len(native["source_combination_members"]) == 180
    assert native["source_candidate_ticket_counts"] == (2,) * 180
    ...
    assert provenance["source_result_selection"].startswith(
        "NO_TARGET_OUTCOME_GRID_WINNER_SELECTION"
    )


def test_wave12_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave12_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    ...
    assert summary["execution_count"] == 55
    assert summary["target_draw_count"] == 55
