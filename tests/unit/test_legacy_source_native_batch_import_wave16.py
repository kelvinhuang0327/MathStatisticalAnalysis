"""Causal batch tests for the sixteenth source-native wave."""
...
def test_wave16_batch_preserves_causal_closed_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert successful[0]["candidate_k"] == 6
    assert successful[0]["combination_count"] == 1
    assert first_native["candidate_k"] is None
    assert first_native["combination_count"] is None
    assert first_native["source_candidate_k_values"] == (6,)
    assert len(first_native["frozen_support_artifacts"]) == 1
...
def test_wave16_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave16_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
