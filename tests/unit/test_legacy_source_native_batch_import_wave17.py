"""Causal batch tests for the seventeenth source-native wave."""
...
def test_wave17_batch_preserves_causal_closed_and_native_semantics(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    document = materialize_legacy_source_native_wave17_batch(
        database=database,
    ...
    assert [row["status"] for row in executions[:2]] == [
    ...
    for row in successful:
        ...
        by_method[cast(str, native["legacy_method_id"])].append(row)
        assert row["candidate_k"] is None
        assert native["candidate_k"] is None
        assert native["combination_count"] is None
    scientific = by_method[SCIENTIFIC_SMART_RANDOM_METHOD_ID][0]
    smart_multi = by_method[SMART_MULTI_BET_METHOD_ID][0]
    assert scientific["native_ticket_count"] == 7
    assert scientific["combination_count"] == 1
    assert smart_multi["native_ticket_count"] == 6
    assert smart_multi["combination_count"] == 6
    scientific_native = cast(
        dict[str, Any],
        scientific["native_generation"],
    )
    smart_native = cast(
        dict[str, Any],
        smart_multi["native_generation"],
    )
    assert scientific_native["source_candidate_ticket_counts"] == ()
    assert len(scientific_native["frozen_support_artifacts"]) == 2
    assert smart_native["source_candidate_ticket_counts"] == (
        6,
        6,
        43,
        0,
    ...
    assert len(smart_native["frozen_support_artifacts"]) == 1
...
def test_wave17_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave17_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
