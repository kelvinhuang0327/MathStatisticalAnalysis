"""Causal batch tests for the twentieth source-native wave."""
...
def test_wave20_batch_preserves_causal_closed_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert row["combination_count"] == 4
    assert row["native_ticket_count"] == 5
    native = cast(dict[str, Any], row["native_generation"])
    assert native["legacy_method_id"] == ZONE_BALANCE_500_METHOD_ID
    assert native["candidate_k"] is None
    assert native["combination_count"] is None
    assert native["native_duplicate_ticket_count"] == 4
    assert native["source_candidate_ticket_counts"] == (1, 1, 1, 1)
...
def test_wave20_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave20_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
