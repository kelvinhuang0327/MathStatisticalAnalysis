"""Causal batch tests for the twenty-second source-native wave."""
...
def test_wave22_batch_preserves_causal_closed_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert native["legacy_method_id"] == SMART_2BET_METHOD_ID
    assert native["candidate_k"] is None
    assert native["combination_count"] is None
    assert native["native_duplicate_ticket_count"] == 0
...
def test_wave22_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave22_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
