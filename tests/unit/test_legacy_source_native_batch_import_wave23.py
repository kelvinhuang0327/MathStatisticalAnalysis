"""Causal batch tests for the twenty-third source-native wave."""
...
def test_wave23_batch_preserves_causal_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    by_method = {
        ...
        for row in successful[:2]
    }
    assert by_method[FIVE_ME_METHOD_ID]["native_ticket_count"] == 5
    assert by_method[FIVE_ME_METHOD_ID]["combination_count"] == 5
    assert by_method[TME_METHOD_ID]["native_ticket_count"] == 3
    assert by_method[TME_METHOD_ID]["combination_count"] == 3
    assert all(row["candidate_k"] is None for row in successful)
...
def test_wave23_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave23_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
