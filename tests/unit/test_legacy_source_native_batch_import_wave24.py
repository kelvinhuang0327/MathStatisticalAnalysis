"""Causal batch tests for the twenty-fourth source-native wave."""
...
def fixture_database_wave24(path: Path) -> str:
...
def test_wave24_batch_preserves_causal_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert len(executions) == 240
    assert [row["status"] for row in executions[:6]] == [
    ...
    assert by_method[TWO_BET_FINAL_METHOD_ID]["native_ticket_count"] == 2
    assert by_method[TWO_BET_FINAL_METHOD_ID]["candidate_k"] == 15
    assert by_method[TWO_BET_FINAL_METHOD_ID]["combination_count"] == 3
    assert by_method[DCB_METHOD_ID]["native_ticket_count"] == 3
    assert by_method[DCB_METHOD_ID]["combination_count"] == 4
    assert by_method[FOUR_BET_DCB_METHOD_ID]["native_ticket_count"] == 4
...
def test_wave24_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
...
def test_wave24_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
    ...
    assert summary["execution_count"] == 240
