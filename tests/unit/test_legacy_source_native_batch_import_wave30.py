"""Causal batch tests for the thirtieth source-native wave."""
...
def test_wave30_batch_preserves_causal_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert all(
        ...
        and row["combination_count"] == 10
        and row["native_ticket_count"] == 10
        and len(cast(list[object], row["ordered_portfolio"])) == 20
        for row in successful
    )
    native = cast(dict[str, Any], successful[0]["native_generation"])
    assert native["legacy_method_id"] == TEN_BET_METHOD_ID
    assert native["source_engine_method_count"] == 7
    assert native["source_ewma_variant_count"] == 3
    assert native["numpy_version_pin"] == "numpy==1.26.2"
...
def test_wave30_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
...
def test_wave30_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
