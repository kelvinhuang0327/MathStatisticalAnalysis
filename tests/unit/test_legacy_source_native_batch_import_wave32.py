"""Causal batch tests for the thirty-second source-native wave."""
...
def test_wave32_batch_preserves_causal_and_native_semantics(
    tmp_path: Path,
) -> None:
    ...
    assert all(
        ...
        and row["combination_count"] == 11
        and row["native_ticket_count"] == 11
...
def test_wave32_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
...
def test_wave32_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = fixture_database_wave24(database)
