"""Causal batch tests for the seventh source-native wave."""
...
def test_wave7_batch_preserves_candidate_combination_and_closed_semantics(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    document = materialize_legacy_source_native_wave7_batch(
        database=database,
        expected_database_sha256=database_sha256,
    )

    executions = cast(list[dict[str, Any]], document["executions"])
    assert len(executions) == 175
    successful = [
    ...
    assert by_method[CLUSTER_6_METHOD_ID]["candidate_k"] == 8
    assert by_method[CLUSTER_6_METHOD_ID]["native_ticket_count"] == 6
    assert by_method[CLUSTER_7_METHOD_ID]["candidate_k"] == 9
    assert by_method[CLUSTER_7_METHOD_ID]["native_ticket_count"] == 7
    assert by_method[BEST_HYBRID_METHOD_ID]["candidate_k"] == 8
    assert by_method[BEST_HYBRID_METHOD_ID]["native_ticket_count"] == 7
    assert "candidate_k" not in by_method[APRIORI_PREDICT_METHOD_ID]
    assert "combination_count" not in by_method[
        APRIORI_PREDICT_METHOD_ID
    ]
    assert by_method[APRIORI_BACKTEST_METHOD_ID][
        "combination_count"
    ] == 4
    assert by_method[APRIORI_BACKTEST_METHOD_ID][
        "native_ticket_count"
...
def test_wave7_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    ...
    assert report["target_draw_count"] == 35
...
def test_wave7_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    ...
    assert summary["execution_count"] == 175
    assert summary["target_draw_count"] == 35
