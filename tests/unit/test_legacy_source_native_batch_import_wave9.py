"""Causal batch tests for the ninth source-native wave."""
...
def test_wave9_batch_preserves_native_configuration_and_closed_semantics(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    document = materialize_legacy_source_native_wave9_batch(
        database=database,
        expected_database_sha256=database_sha256,
    )

    executions = cast(list[dict[str, Any]], document["executions"])
    assert len(executions) == 615
    successful = [
    ...
    assert len(successful) == 464
    assert len(closed) == 151
    assert all(
    ...
    cluster = by_method[CLUSTER_PIVOT_BENCHMARK_METHOD_ID]
    assert cluster["combination_count"] == 7
    assert cast(int, cluster["native_ticket_count"]) in (17, 19)
    orthogonal = by_method[TRUE_ORTHOGONAL_METHOD_ID]
    assert orthogonal["combination_count"] == 9
    assert cast(int, orthogonal["native_ticket_count"]) >= 18
    p0p1 = by_method[P0P1_UPGRADE_METHOD_ID]
    assert p0p1["combination_count"] == 4
    assert p0p1["native_ticket_count"] == 10
    p0p1_native = cast(dict[str, Any], p0p1["native_generation"])
    assert p0p1_native["candidate_k"] is None
    assert p0p1_native["combination_count"] is None
    assert p0p1_native["source_candidate_k_values"]
...
def test_wave9_batch_passes_complete_universe_evaluator(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
...
def test_wave9_cli_is_registered_and_refuses_overwrite(
    tmp_path: Path,
) -> None:
    database = tmp_path / "legacy.db"
    database_sha256 = _fixture_database(database)
    ...
    assert summary["execution_count"] == 615
