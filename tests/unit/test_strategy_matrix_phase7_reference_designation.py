...
def test_source_commit_is_full_sha1_and_is_the_files_only_commit(
    ...
    assert SOURCE_COMMIT in commits, (
        "expected the pinned commit to be among the source file's commits; got "
        f"{commits!r}"
    )
    blobs = {
        subprocess.run(
    ...
    assert len(blobs) == 1, (
        "expected every commit touching the source file to carry identical "
        f"content (else pinning to just one is unsafe); found {len(blobs)} "
        f"distinct blobs across {commits!r}"
    )
