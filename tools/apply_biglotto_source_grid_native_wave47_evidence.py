#!/usr/bin/env python3
"""Apply wave-47 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "aeaae35577c58b7e9e9b981f65480da471472670db1f0f170237b328ac543348"
EXPECTED_EVIDENCE_FILE_SHA256 = "226ef87bd7e97e9a74dc9bac5194ccc2be5b869528fb191c7c0ea16f0d0fa7fb"
EVIDENCE_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE47_EVIDENCE_V1"
EVIDENCE_ARTIFACT_NAME = "biglotto_legacy_source_grid_native_wave47_evidence_v1.json"
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-47 evidence is inconsistent."""
...
def _validate_evidence(
    ...
    if (
        ...
        raise CatalogOverlayError("wave-47 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-47 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if method_id not in SUPPORTED_SOURCE_NATIVE_WAVE47_METHODS or method_id in by_method:
            raise CatalogOverlayError("wave-47 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE47_METHOD[typed_method_id]
        expected_configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE47_METHOD[
            typed_method_id
        ]
        if (
            ...
            raise CatalogOverlayError(f"wave-47 strategy evidence changed: {method_id}")
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE47_METHODS):
        raise CatalogOverlayError("wave-47 strategy evidence is incomplete")
    alias = evidence.get("alias_disposition")
    if not isinstance(alias, dict):
        raise CatalogOverlayError("wave-47 alias evidence changed")
    typed_alias = cast(dict[str, Any], alias)
    if (
        typed_alias.get("alias_method_id") != STABILITY_ALIAS_METHOD_ID
        or typed_alias.get("canonical_method_id") != STABILITY_ALIAS_TARGET_METHOD_ID
        or typed_alias.get("overlapping_causal_output_case_count") != 1649
        or typed_alias.get("output_mismatch_count") != 0
        or typed_alias.get("status") != "DUPLICATE_ALIAS"
    ):
        raise CatalogOverlayError("wave-47 alias proof changed")
    return by_method, typed_alias


def apply_wave47_evidence(
    *,
    base_catalog_path: Path,
    evidence_path: Path,
) -> dict[str, object]:
    """Overlay seven BACKTESTED rows and one DUPLICATE_ALIAS row."""
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE47_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-47 evidence leaves the validated universe")
        ...
        record.update(
            {
                ...
                "status_reason": (
                    ...
                    f"{native_count} native positional ticket(s), "
                ...
                "ticket_duplicate_semantics": (
                    ...
                    "INCLUDING_DUPLICATES_BEFORE_CHECKSUMMED_ORDERED_20_"
                    "DERIVATION"
                ),
    ...
    alias_record = record_by_method.get(STABILITY_ALIAS_METHOD_ID)
    canonical_record = record_by_method.get(STABILITY_ALIAS_TARGET_METHOD_ID)
    if (
        ...
        raise CatalogOverlayError("wave-47 duplicate alias leaves the validated universe")
    alias_record.update(
        {
            ...
            "status_reason": (
                ...
                "backtest_big_lotto_orthogonal_5bet.py at all 1,649 "
                "overlapping causal cutoffs with zero positional ticket "
                "mismatches. Ranking it independently would double count "
                "the same selection method. Compact evidence SHA-256 is "
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE47_FULL_PREFIX_CAUSAL_BACKTEST_AND_"
                "STABILITY_PORTFOLIO_ALIAS_PROOF"
            ),
        }
    )
    status_counts = Counter(
        cast(
            str,
            cast(dict[str, Any], item)["reproduction_status"],
        )
        for item in records
    )
