#!/usr/bin/env python3
"""Apply wave-49 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "5c6f759bdb23b7ad19814611bfc495eda8a1e992446d4a88209351daf79ae0cc"
...
EVIDENCE_ARTIFACT_NAME = "biglotto_legacy_source_grid_native_wave49_evidence_v1.json"
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-49 evidence is inconsistent."""
...
def _validate_evidence(evidence: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if (
        ...
        raise CatalogOverlayError("wave-49 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-49 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if method_id not in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS or method_id in by_method:
            raise CatalogOverlayError("wave-49 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[
            typed_method_id
        ]
        expected_configuration_count = (
            SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[typed_method_id]
        )
        if (
            ...
            raise CatalogOverlayError(f"wave-49 strategy evidence changed: {method_id}")
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS):
        raise CatalogOverlayError("wave-49 strategy evidence is incomplete")
    return by_method


def apply_wave49_evidence(
    *,
    base_catalog_path: Path,
    evidence_path: Path,
) -> dict[str, object]:
    """Overlay three BACKTESTED rows."""
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE49_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-49 evidence leaves the validated universe")
        configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[
            method_id
        ]
        native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE49_METHOD[method_id]
        record.update(
            {
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE49_AUTO_DISCOVERY_SIGNAL_GRID_"
                "AND_FOURIER_CAUSAL_BACKTEST"
            ),
        }
    )
