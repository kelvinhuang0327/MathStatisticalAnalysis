#!/usr/bin/env python3
"""Apply wave-50 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "8677e817497c0daba496f1169ea708575f738fe7931d4ddfdd2a9c4123b92059"
EXPECTED_EVIDENCE_FILE_SHA256 = (
    "2d0a142046b9fff65446daa8d2d2144fd959d7c3c00120eac18f2a03dbabf425"
)
EVIDENCE_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE50_EVIDENCE_V1"
EVIDENCE_ARTIFACT_NAME = (
    "biglotto_legacy_source_grid_native_wave50_evidence_v1.json"
)
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-50 evidence is inconsistent."""
...
def _validate_evidence(evidence: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if (
        ...
        != "d4688d7bfa91b84e5234ab42b443313c0e7c0890429b4e801c2e95d0efdc901a"
        ...
        raise CatalogOverlayError("wave-50 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-50 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS
            or method_id in by_method
        ):
            raise CatalogOverlayError("wave-50 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE50_METHOD[
            typed_method_id
        ]
        expected_configuration_count = (
            SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE50_METHOD[typed_method_id]
        )
        if (
            ...
            raise CatalogOverlayError(
                f"wave-50 strategy evidence changed: {method_id}"
            )
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS):
        raise CatalogOverlayError("wave-50 strategy evidence is incomplete")
    return by_method


def apply_wave50_evidence(
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE50_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-50 evidence leaves the validated universe")
        configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE50_METHOD[
            method_id
        ]
        native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE50_METHOD[method_id]
        record.update(
            {
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE50_COVERING_AND_EXHAUSTIVE_FEATURE_"
                "SWEEP_CAUSAL_BACKTEST"
            ),
        }
    )
