#!/usr/bin/env python3
"""Apply wave-51 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "fe728b48e139137d778fd26f163322d56e817f7e981fabd14afa9e3cec00b963"
EXPECTED_EVIDENCE_FILE_SHA256 = (
    "292dee4f94a057b7ba44a92bcdce67ee11662f2f2f2e6fdc436bad7e965bb802"
)
EVIDENCE_SCHEMA_VERSION = "BIG_LOTTO_LEGACY_SOURCE_GRID_NATIVE_WAVE51_EVIDENCE_V1"
EVIDENCE_ARTIFACT_NAME = (
    "biglotto_legacy_source_grid_native_wave51_evidence_v1.json"
)
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-51 evidence is inconsistent."""
...
def _validate_evidence(evidence: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if (
        ...
        != "7eb4592fc7d1d353bd59e3615b29f50fb1821f6f4defb96b5978b684e85952db"
        ...
        raise CatalogOverlayError("wave-51 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-51 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE51_METHODS
            or method_id in by_method
        ):
            raise CatalogOverlayError("wave-51 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE51_METHOD[
            typed_method_id
        ]
        expected_configuration_count = (
            SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE51_METHOD[typed_method_id]
        )
        if (
            ...
            raise CatalogOverlayError(
                f"wave-51 strategy evidence changed: {method_id}"
            )
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE51_METHODS):
        raise CatalogOverlayError("wave-51 strategy evidence is incomplete")
    return by_method


def apply_wave51_evidence(
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE51_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-51 evidence leaves the validated universe")
        configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE51_METHOD[
            method_id
        ]
        native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE51_METHOD[method_id]
        record.update(
            {
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE51_CLUSTER_AND_DEVIATION_EXTREME_"
                "SEEDED_CAUSAL_BACKTEST"
            ),
        }
    )
