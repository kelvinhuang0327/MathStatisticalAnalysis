#!/usr/bin/env python3
"""Apply wave-52 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "ada478ff7f67880ef1d6907142d3a47afd51c10aeb029a7f3f79eb779c2fb111"
...
EVIDENCE_ARTIFACT_NAME = (
    "biglotto_legacy_source_grid_native_wave52_evidence_v1.json"
)
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-52 evidence is inconsistent."""
...
def _validate_evidence(evidence: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if (
        ...
        raise CatalogOverlayError("wave-52 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-52 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS
            or method_id in by_method
        ):
            raise CatalogOverlayError("wave-52 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE52_METHOD[
            typed_method_id
        ]
        expected_configuration_count = (
            SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE52_METHOD[typed_method_id]
        )
        if (
            ...
            raise CatalogOverlayError(
                f"wave-52 strategy evidence changed: {method_id}"
            )
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS):
        raise CatalogOverlayError("wave-52 strategy evidence is incomplete")
    return by_method


def apply_wave52_evidence(
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE52_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-52 evidence leaves the validated universe")
        configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE52_METHOD[
            method_id
        ]
        native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE52_METHOD[method_id]
        record.update(
            {
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE52_FEATURE_DISCOVERY_AND_FROZEN_RECIPE_"
                "EXPLICITLY_SEEDED_CAUSAL_BACKTEST"
            ),
        }
    )
