#!/usr/bin/env python3
"""Apply wave-53 source-grid evidence to the full strategy catalog."""
...
EXPECTED_EVIDENCE_SHA256 = "08e62eb8a3a791bb1dd0d7ff123f77f47f7bbfe0749ba53850c7a05e79c17f60"
...
EVIDENCE_ARTIFACT_NAME = (
    "biglotto_legacy_source_grid_native_wave53_evidence_v1.json"
)
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-53 evidence is inconsistent."""
...
def _validate_evidence(evidence: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if (
        ...
        raise CatalogOverlayError("wave-53 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("wave-53 strategy evidence changed")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS
            or method_id in by_method
        ):
            raise CatalogOverlayError("wave-53 strategy method set changed")
        typed_method_id = cast(str, method_id)
        expected_ok = EXPECTED_OK_COUNTS[typed_method_id]
        expected_native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[
            typed_method_id
        ]
        expected_configuration_count = (
            SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[typed_method_id]
        )
        if (
            ...
            raise CatalogOverlayError(
                f"wave-53 strategy evidence changed: {method_id}"
            )
        by_method[typed_method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS):
        raise CatalogOverlayError("wave-53 strategy evidence is incomplete")
    return by_method


def apply_wave53_evidence(
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE53_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError("wave-53 evidence leaves the validated universe")
        configuration_count = SOURCE_CONFIGURATION_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[
            method_id
        ]
        native_count = NATIVE_TICKET_COUNT_BY_SOURCE_NATIVE_WAVE53_METHOD[method_id]
        record.update(
            {
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE53_NESTED_VERIFIED_STRATEGIES_AND_"
                "RGF_FORMULA_CAUSAL_BACKTEST"
            ),
        }
    )
