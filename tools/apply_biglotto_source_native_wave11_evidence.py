#!/usr/bin/env python3
"""Apply the eleventh source-native evidence wave to a validated catalog."""
...
EXPECTED_SUCCESS_COUNTS = {
    EXHAUSTIVE_NBET_METHOD_ID: 1649,
    MUST_HIT_METHOD_ID: 2099,
}
...
class CatalogOverlayError(ValueError):
    """The base catalog or wave-11 evidence violates the overlay contract."""
...
def _validate_evidence(
    ...
    if (
        ...
        raise CatalogOverlayError("wave-11 evidence identity changed")
    for key in (
        "input_raw_sha256",
    ...
    if (
        ...
        raise CatalogOverlayError(
            "wave-11 reproducibility/parity evidence changed"
        )
    ...
    if not isinstance(rows_raw, list):
        raise CatalogOverlayError(
            "wave-11 evidence must contain two strategies"
        )
    ...
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError(
                "wave-11 evidence strategy is invalid"
            )
        ...
        if (
            ...
            or method_id not in SUPPORTED_SOURCE_NATIVE_WAVE11_METHODS
        ):
            raise CatalogOverlayError(
                "wave-11 strategy identity is invalid"
            )
        if (
            ...
            != MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE11_METHOD[method_id]
            ...
            != RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE11_METHOD[method_id]
            ...
            or row.get("candidate_pool_size_patterns")
            != EXPECTED_CANDIDATE_POOL_SIZE_PATTERNS[method_id]
        ):
            raise CatalogOverlayError(
                f"wave-11 strategy evidence changed: {method_id}"
            )
        by_method[method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE11_METHODS):
        raise CatalogOverlayError(
            "wave-11 evidence method coverage changed"
        )
    return by_method
...
def _candidate_semantics(method_id: str) -> str:
    if method_id == MUST_HIT_METHOD_ID:
        return (
            "FROZEN_SOURCE_TOP6_TOP10_TOP15_CANDIDATE_NUMBER_POOLS_"
            "DISTINCT_FROM_ONE_LEGAL_NATIVE_TICKET"
        )
...
def _combination_semantics(method_id: str) -> str:
    if method_id == EXHAUSTIVE_NBET_METHOD_ID:
        return (
            "FROZEN_SOURCE_26_BIG_LOTTO_STRATEGY_CONFIGURATIONS_"
            "DISTINCT_FROM_65_POSITIONAL_NATIVE_TICKETS"
        )
    return (
        "FROZEN_SOURCE_3_TOP_N_PARAMETER_CONFIGURATIONS_DISTINCT_"
        "FROM_ONE_LEGAL_TOP6_NATIVE_TICKET"
    )


def apply_wave11_evidence(
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE11_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError(
                "wave-11 evidence leaves the validated base universe"
            )
        record.update(
            {
                ...
                "status_reason": (
                    "Exact frozen-commit function parity passed 6 "
                ...
                "ticket_duplicate_semantics": (
                    "PRESERVE_FROZEN_SOURCE_POSITIONAL_DUPLICATES"
                    if method_id == EXHAUSTIVE_NBET_METHOD_ID
                    else "NONE_SINGLE_LEGAL_NATIVE_TICKET"
                ),
                "ticket_order_semantics": (
                    "FROZEN_SOURCE_CONFIGURATION_AND_BET_LOOP_ORDER_"
                    "BEFORE_ORDERED_20_CONSTRUCTION"
                    if method_id == EXHAUSTIVE_NBET_METHOD_ID
                    else "FROZEN_SOURCE_TOP6_FREQUENCY_ORDER_BEFORE_"
                    "CANONICAL_LEGAL_TICKET_AND_ORDERED_20_CONSTRUCTION"
                ),
                "unranked_reason": (
