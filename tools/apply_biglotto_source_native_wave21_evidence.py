#!/usr/bin/env python3
"""Apply wave-21 source-native evidence to the full BIG_LOTTO catalog."""
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-21 evidence is inconsistent."""
...
def _validate_evidence(
    ...
    if (
        ...
        != {"0": 2136, "1": 12}
        or strategy.get("danger_number_count_distribution")
        != {"0": 1936, "1": 207, "2": 5}
        or strategy.get("zone_retry_count") != 59
        or strategy.get("zone_fallback_count") != 0
...
def apply_wave21_evidence(
    *,
    ...
    record = record_by_method.get(POST_SELECTION_FILTER_METHOD_ID)
    if (
        record is None
    ...
    record.update(
        {
            ...
            "combination_count_semantics": (
                "TWO_FROZEN_SELECTION_BRANCHES_DISTINCT_FROM_TWO_"
            ...
            "status_reason": (
                "Exact frozen-statement parity passed five pinned causal "
                "history cutoffs plus a constructed danger-streak case, "
                "with UnifiedPredictionEngine zone-balance support pinned. "
                "The Frequency-50 post-filter ticket and Zone-Balance-500 "
                "ticket, including danger-triggered 510 retry and source "
                "fallback semantics, remain positional. 2148 causal "
                "executions completed and one insufficient-history closure "
                "remained explicit. Compact evidence SHA-256 is "
                f"{evidence_sha256}."
            ),
            "ticket_duplicate_semantics": (
                "PRESERVE_BOTH_POSITIONAL_TICKETS_WHEN_FREQUENCY_AND_"
                "ZONE_BRANCHES_EMIT_THE_SAME_TICKET"
            ),
            "ticket_order_semantics": (
                "FROZEN_FREQUENCY_50_POST_FILTER_THEN_ZONE_BALANCE_"
                "500_OR_510_ORDER_BEFORE_ORDERED_20_CONSTRUCTION"
            ),
    ...
    source_artifacts.append(
        {
            ...
            "evidence_role": (
                "SOURCE_NATIVE_WAVE21_POST_SELECTION_FILTER_CAUSAL_"
                "BACKTEST"
            ),
        }
    )
