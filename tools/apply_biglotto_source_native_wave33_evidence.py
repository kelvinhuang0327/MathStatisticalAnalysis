#!/usr/bin/env python3
"""Apply wave-33 source-native evidence to the full BIG_LOTTO catalog."""
...
EVIDENCE_ARTIFACT_NAME = (
    "biglotto_legacy_source_native_wave33_evidence_v1.json"
)
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-33 evidence is inconsistent."""
...
def _validate_evidence(
    ...
    if (
        ...
        raise CatalogOverlayError("wave-33 evidence identity changed")
    rows = cast(list[object], evidence.get("strategies", []))
    if len(rows) != 1 or not isinstance(rows[0], dict):
        raise CatalogOverlayError(
            "wave-33 evidence must contain one strategy"
        )
    row = cast(dict[str, Any], rows[0])
    if (
        row.get("legacy_method_id") != FEASIBILITY_METHOD_ID
        ...
        or row.get("candidate_pool_size_distribution")
        != {"12": 2148}
        or row.get("statistical_fallback_execution_count") != 0
    ):
        raise CatalogOverlayError("wave-33 strategy identity changed")
    ...
    if (
        ...
        raise CatalogOverlayError("wave-33 parity evidence changed")
    return row


def apply_wave33_evidence(
    *,
    base_catalog_path: Path,
    evidence_path: Path,
) -> dict[str, object]:
    """Overlay the validated wave-33 BACKTESTED disposition."""
    ...
    if evidence_sha256 != EXPECTED_EVIDENCE_SHA256:
        raise CatalogOverlayError("wave-33 evidence file changed")
    ...
    record = record_by_method.get(FEASIBILITY_METHOD_ID)
    if (
        record is None
        ...
        raise CatalogOverlayError(
            "wave-33 evidence leaves the validated universe"
        )
    record.update(
        {
            "candidate_k_semantics": (
                "NULL_NO_COMBINATORIAL_CANDIDATE_K_WHILE_FROZEN_"
                "TOP12_COUNTER_POOL_IS_RETAINED_SEPARATELY"
            ),
            "combination_count_semantics": (
                "FROZEN_SIX_BENCHMARK_CONFIGURATIONS_DISTINCT_FROM_"
                "EIGHT_NATIVE_TICKETS_AND_ORDERED_20"
            ),
            ...
            "status_reason": (
                "Frozen wrapper AST parity covered 65 causal histories "
                "and preserved four single-ticket configurations, "
                "Markov-plus-Deviation order, Top-12 Counter insertion "
                "ties, two disjoint Top-12 slices, source-seeded "
                "statistical randomness, and native duplicates. 2148 "
                "causal executions completed and one minimum-history "
                "closure remained explicit. Compact evidence SHA-256 "
                f"is {evidence_sha256}."
            ),
            "ticket_duplicate_semantics": (
                "PRESERVE_ALL_EIGHT_FROZEN_POSITIONAL_TICKETS_"
                "INCLUDING_CROSS_CONFIGURATION_DUPLICATES"
            ),
            "ticket_order_semantics": (
                "FOUR_SINGLE_CONFIGS_THEN_MARKOV_DEVIATION_PAIR_THEN_"
                "TOP12_SLICES_0_6_AND_6_12_BEFORE_ORDERED_20"
            ),
