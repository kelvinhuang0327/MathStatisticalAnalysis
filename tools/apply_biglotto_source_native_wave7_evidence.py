"""Apply the seventh source-native evidence wave to a validated catalog."""
...
BASE_CATALOG_SHA256 = (
    "4e8f8414e2fceaafe3954d70f6db6d38b60b698b0c95fd115d63a375089d91e6"
)
EVIDENCE_SCHEMA_VERSION = (
    "BIG_LOTTO_LEGACY_SOURCE_NATIVE_WAVE7_EVIDENCE_V1"
)
...
EXPECTED_SUCCESS_COUNTS = {
    "tools/predict_biglotto_6bets_cluster.py": 2146,
    "tools/predict_biglotto_7bets_cluster.py": 2146,
    "tools/predict_biglotto_apriori.py": 2140,
    "tools/backtest_apriori.py": 2148,
    "tools/predict_biglotto_best.py": 2146,
}
EXPECTED_CLOSED_COUNTS = {
    "tools/predict_biglotto_6bets_cluster.py": {
        "CLOSED_EXECUTION_ERROR": 2,
        "CLOSED_INSUFFICIENT_HISTORY": 1,
    },
    "tools/predict_biglotto_7bets_cluster.py": {
        "CLOSED_EXECUTION_ERROR": 2,
        "CLOSED_INSUFFICIENT_HISTORY": 1,
    },
    "tools/predict_biglotto_apriori.py": {
        "CLOSED_EXECUTION_ERROR": 8,
        "CLOSED_INSUFFICIENT_HISTORY": 1,
    },
...
EXPECTED_NATIVE_COUNT_VALUES = {
    "tools/predict_biglotto_6bets_cluster.py": [6],
    "tools/predict_biglotto_7bets_cluster.py": [7],
    "tools/predict_biglotto_apriori.py": [2, 3, 4, 5, 6, 7],
    "tools/backtest_apriori.py": [13],
    "tools/predict_biglotto_best.py": [7],
}
_SHA256 = re.compile(r"[0-9a-f]{64}", flags=re.ASCII)


class CatalogOverlayError(ValueError):
    """The base catalog or wave-7 evidence violates the overlay contract."""
...
def _validate_evidence(
    ...
    if (
        ...
        != (
            "EXECUTION_LEVEL_INTERNAL_CLUSTER_CENTER_K_DISTINCT_"
        ...
        raise CatalogOverlayError("wave-7 evidence identity changed")
    for key in (
        "input_raw_sha256",
    ...
    if not isinstance(reproducibility_raw, dict):
        raise CatalogOverlayError(
            "wave-7 reproducibility evidence changed"
        )
    ...
    if not isinstance(parity_raw, dict):
        raise CatalogOverlayError(
            "wave-7 frozen-source parity evidence changed"
        )
    parity = cast(dict[str, Any], parity_raw)
    if (
        parity.get("status") != "PASS"
        or parity.get("case_count") != 15
    ...
    if not isinstance(rows_raw, list):
        raise CatalogOverlayError(
            "wave-7 evidence must contain exactly five strategies"
        )
    ...
    for index, candidate in enumerate(rows):
        if not isinstance(candidate, dict):
            raise CatalogOverlayError(
                f"wave-7 strategies[{index}] must be an object"
            )
        ...
        if (
            ...
            or method_id not in SUPPORTED_SOURCE_NATIVE_WAVE7_METHODS
        ):
            raise CatalogOverlayError(
                "wave-7 strategy identity is invalid"
            )
        if (
            ...
            != MINIMUM_HISTORY_BY_SOURCE_NATIVE_WAVE7_METHOD[method_id]
            ...
            != RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE7_METHOD[method_id]
            or row.get("source_history_order")
            != SOURCE_HISTORY_ORDER_BY_SOURCE_NATIVE_WAVE7_METHOD[
                method_id
            ]
            or row.get("candidate_k")
            != CANDIDATE_K_BY_SOURCE_NATIVE_WAVE7_METHOD[method_id]
            ...
            raise CatalogOverlayError(
                "wave-7 execution semantics changed"
            )
        by_method[method_id] = row
    if set(by_method) != set(SUPPORTED_SOURCE_NATIVE_WAVE7_METHODS):
        raise CatalogOverlayError("wave-7 evidence omits a frozen method")
    return by_method
...
def _candidate_semantics(method_id: str) -> str:
    ...
    if candidate_k is None:
        return "NOT_APPLICABLE_NO_SINGLE_TOP_K_CANDIDATE_CONTRACT"
    return (
        f"FROZEN_INTERNAL_CLUSTER_CENTER_CANDIDATE_K_{candidate_k}_"
        "DISTINCT_FROM_NATIVE_TICKETS"
    )


def _combination_semantics(method_id: str) -> str:
    count = SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE7_METHOD[
        method_id
    ]
    if count is None:
    ...
    *,
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE7_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError(
                "wave-7 evidence leaves the validated base universe"
            )
        random_protocol = (
            RANDOM_PROTOCOL_BY_SOURCE_NATIVE_WAVE7_METHOD[method_id]
        )
        random_note = (
            ...
            if random_protocol == "NONE_DETERMINISTIC"
                ...
                " The unpreserved legacy global RNG stream is not "
                "claimed recovered; frozen random call order uses a "
                "versioned target-stable MT19937 replacement."
            )
        )
        ...
        record.update(
            {
                ...
                "status_reason": (
                    "Exact frozen-Git-byte function parity passed 15 "
                    f"cross-method cases; {EXPECTED_SUCCESS_COUNTS[method_id]} "
                    "causal executions completed while frozen early-output "
                    f"closures remained explicit as {closed}."
