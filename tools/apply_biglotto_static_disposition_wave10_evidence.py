#!/usr/bin/env python3
"""Apply wave-10 frozen HTTP-only closures to the BIG_LOTTO catalog."""
...
class CatalogOverlayError(ValueError):
    """The catalog or wave-10 disposition evidence is inconsistent."""
...
def _validate_evidence(
    ...
    if (
        ...
        raise CatalogOverlayError("wave-10 evidence identity changed")
    rows_raw = evidence.get("dispositions")
    if not isinstance(rows_raw, list):
        raise CatalogOverlayError(
            "wave-10 evidence must contain four dispositions"
        )
    ...
    for candidate in rows:
        if not isinstance(candidate, dict):
            raise CatalogOverlayError(
                "wave-10 disposition must be an object"
            )
        ...
        if (
            ...
            raise CatalogOverlayError(
                "wave-10 disposition identity changed"
            )
        ...
        if len(facts) < 3 or any(
            ...
            raise CatalogOverlayError(
                f"wave-10 decisive facts changed: {method_id}"
            )
        _validate_digest(
    ...
    if set(by_method) != set(CLOSED_METHODS):
        raise CatalogOverlayError("wave-10 evidence omits a method")
    return by_method


def apply_wave10_evidence(
    *,
    ...
    for method_id in CLOSED_METHODS:
        ...
        if (
            ...
            raise CatalogOverlayError(
                "wave-10 evidence leaves the validated universe"
            )
        record.update(
            {
                ...
                "status_reason": (
                    ...
                    "wave-10 disposition evidence SHA-256 is "
                    f"{evidence_sha256}."
                ),
