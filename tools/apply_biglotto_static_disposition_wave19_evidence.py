#!/usr/bin/env python3
"""Apply wave-19 frozen-source closures to the BIG_LOTTO catalog."""

from __future__ import annotations

import argparse
import hashlib
import json
import re
from pathlib import Path
from typing import Any, cast

from lottolab.domain.biglotto_full_strategy_catalog import (
    CATALOG_POLICY_VERSION,
    CATALOG_SCHEMA_VERSION,
)

BASE_CATALOG_SHA256 = (
    "048eab0b352a030a3f38634b9c5122142e5a0749e4ae569104dfeb0b85065736"
)
EVIDENCE_SCHEMA_VERSION = (
    "BIG_LOTTO_STATIC_DISPOSITION_WAVE19_EVIDENCE_V1"
)
FROZEN_SOURCE_COMMIT = "49a25effa62fc24f40789c16be6f11bdfb41a4a9"
REVIEW_POLICY_VERSION = "BIG_LOTTO_FROZEN_SOURCE_DISPOSITION_REVIEW_V6"
EXPECTED_EVIDENCE_SHA256 = (
    "4ef9ff56ae02fce17b19134fe872c316fab4d5536b1bc42dd668fef199f51982"
)
NON_BIGLOTTO_REASON = "NON_BIG_LOTTO_FIXED_5_OF_39_SOURCE"
SYNTHETIC_HARNESS_REASON = (
    "SYNTHETIC_INPUT_IMPORTED_PREDICTOR_TEST_HARNESS"
)
COMPARATIVE_HARNESS_REASON = (
    "COMPARATIVE_IMPORTED_PREDICTOR_BACKTEST_WITHOUT_"
    "INDEPENDENT_TARGET_PORTFOLIO"
)
CLOSED_METHODS: dict[str, tuple[str, str]] = {
    "tools/backtest_39lotto_comprehensive.py": (
        "c233ed549b92ffd537066a07e3c956966a8fd273d76d8fcbd9e7df5df4bf02ae",
        NON_BIGLOTTO_REASON,
    ),
    "tools/testing/test-optimization-simple.py": (
        "888431e7cfd2a448c13efb1671e45f1209fe3c831bdbdbb882309cbe19dd7713",
        SYNTHETIC_HARNESS_REASON,
    ),
    "tools/testing/test-optimization-b.py": (
        "bb1ae535163a9666f2c7b5fc2326c5cd2a5c8f0d44de736ede9fdce63c808750",
        SYNTHETIC_HARNESS_REASON,
    ),
    "tools/testing/test-all-optimizations.py": (
        "c7bc2a0fe6f948834aa74308bba808908fd9201053f4ce6155b070df9a60952d",
        SYNTHETIC_HARNESS_REASON,
    ),
    "tools/backtest_ml_comprehensive_2025_biglotto.py": (
        "a3e2b5b7e5b3117053385cf59611ce1c542a8f4291b3892b0f98d5cfccbad9a3",
        COMPARATIVE_HARNESS_REASON,
    ),
}
EXPECTED_BASE_STATUS_COUNTS = {
    "BACKTESTED": 43,
    "CLOSED_UNEXECUTABLE": 32,
    "DUPLICATE_ALIAS": 5,
    "OWNER_DECISION_REQUIRED": 141,
}
EXPECTED_OUTPUT_STATUS_COUNTS = {
    "BACKTESTED": 43,
    "CLOSED_UNEXECUTABLE": 37,
    "DUPLICATE_ALIAS": 5,
    "OWNER_DECISION_REQUIRED": 136,
}
_SHA256 = re.compile(r"[0-9a-f]{64}", flags=re.ASCII)


class CatalogOverlayError(ValueError):
    """The catalog or wave-19 disposition evidence is inconsistent."""


def _canonical_bytes(value: object) -> bytes:
    return json.dumps(
        value,
        ensure_ascii=False,
        separators=(",", ":"),
        sort_keys=True,
    ).encode("utf-8")


def _read_json(path: Path) -> tuple[dict[str, Any], str]:
    raw = path.read_bytes()
    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise CatalogOverlayError(f"{path}: invalid JSON") from exc
    if not isinstance(parsed, dict):
        raise CatalogOverlayError(f"{path}: top level must be an object")
    return cast(dict[str, Any], parsed), hashlib.sha256(raw).hexdigest()


def _catalog_hash(document: dict[str, Any]) -> str:
    reduced = {
        key: value
        for key, value in document.items()
        if key != "catalog_sha256"
    }
    return hashlib.sha256(_canonical_bytes(reduced)).hexdigest()


def _validate_evidence(
    evidence: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    if (
        evidence.get("evidence_schema_version")
        != EVIDENCE_SCHEMA_VERSION
        or evidence.get("frozen_source_commit")
        != FROZEN_SOURCE_COMMIT
        or evidence.get("review_policy_version")
        != REVIEW_POLICY_VERSION
        or evidence.get("base_catalog_sha256")
        != BASE_CATALOG_SHA256
    ):
        raise CatalogOverlayError("wave-19 evidence identity changed")
    rows_raw = evidence.get("dispositions")
    if (
        not isinstance(rows_raw, list)
        or len(cast(list[object], rows_raw)) != 5
    ):
        raise CatalogOverlayError(
            "wave-19 evidence must contain five dispositions"
        )
    by_method: dict[str, dict[str, Any]] = {}
    for candidate in cast(list[object], rows_raw):
        if not isinstance(candidate, dict):
            raise CatalogOverlayError(
                "wave-19 disposition must be an object"
            )
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        facts_raw = row.get("decisive_source_facts")
        if type(method_id) is not str or method_id not in CLOSED_METHODS:
            raise CatalogOverlayError(
                "wave-19 disposition method changed"
            )
        expected_source, expected_reason = CLOSED_METHODS[method_id]
        if (
            method_id in by_method
            or row.get("reproduction_status")
            != "CLOSED_UNEXECUTABLE"
            or row.get("reason_code") != expected_reason
            or row.get("source_sha256") != expected_source
            or type(row.get("source_blob_id")) is not str
            or len(cast(str, row["source_blob_id"])) != 40
            or type(row.get("source_byte_size")) is not int
            or cast(int, row["source_byte_size"]) <= 0
            or type(row.get("status_reason")) is not str
            or not isinstance(facts_raw, list)
            or len(cast(list[object], facts_raw)) < 3
            or _SHA256.fullmatch(expected_source) is None
        ):
            raise CatalogOverlayError(
                "wave-19 disposition identity changed"
            )
        by_method[method_id] = row
    if set(by_method) != set(CLOSED_METHODS):
        raise CatalogOverlayError("wave-19 evidence omits a closure")
    return by_method


def apply_wave19_evidence(
    *,
    base_catalog_path: Path,
    evidence_path: Path,
) -> dict[str, object]:
    catalog, _raw_catalog_sha256 = _read_json(base_catalog_path)
    evidence, evidence_sha256 = _read_json(evidence_path)
    if (
        catalog.get("catalog_schema_version") != CATALOG_SCHEMA_VERSION
        or catalog.get("catalog_policy_version")
        != CATALOG_POLICY_VERSION
        or catalog.get("catalog_sha256") != BASE_CATALOG_SHA256
        or _catalog_hash(catalog) != BASE_CATALOG_SHA256
        or catalog.get("status_counts")
        != EXPECTED_BASE_STATUS_COUNTS
        or catalog.get("full_universe_complete") is not False
        or catalog.get("frozen_source_commit")
        != FROZEN_SOURCE_COMMIT
    ):
        raise CatalogOverlayError("base catalog identity changed")
    if evidence_sha256 != EXPECTED_EVIDENCE_SHA256:
        raise CatalogOverlayError("wave-19 evidence file changed")
    dispositions = _validate_evidence(evidence)

    records_raw = catalog.get("records")
    if not isinstance(records_raw, list):
        raise CatalogOverlayError("base catalog records changed")
    record_by_method: dict[str, dict[str, Any]] = {}
    for candidate in cast(list[object], records_raw):
        if not isinstance(candidate, dict):
            raise CatalogOverlayError("base catalog record is invalid")
        record = cast(dict[str, Any], candidate)
        method_id = record.get("legacy_method_id")
        if isinstance(method_id, str):
            record_by_method[method_id] = record
    if len(record_by_method) != 221:
        raise CatalogOverlayError("base catalog records changed")

    for method_id, evidence_row in dispositions.items():
        record = record_by_method.get(method_id)
        reason_code = cast(str, evidence_row["reason_code"])
        if (
            record is None
            or record.get("reproduction_status")
            != "OWNER_DECISION_REQUIRED"
            or record.get("source_commit") != FROZEN_SOURCE_COMMIT
            or record.get("source_sha256")
            != evidence_row.get("source_sha256")
            or record.get("source_blob_id")
            != evidence_row.get("source_blob_id")
            or record.get("source_byte_size")
            != evidence_row.get("source_byte_size")
        ):
            raise CatalogOverlayError(
                "wave-19 closure leaves the validated universe"
            )
        record.update(
            {
                "candidate_k_semantics": (
                    "NOT_APPLICABLE_CLOSED_UNEXECUTABLE"
                ),
                "combination_count_semantics": (
                    "NOT_APPLICABLE_CLOSED_UNEXECUTABLE"
                ),
                "native_ticket_semantics": (
                    "NO_INDEPENDENT_EXECUTABLE_BIG_LOTTO_TARGET_PORTFOLIO"
                ),
                "reproduction_status": "CLOSED_UNEXECUTABLE",
                "status_reason": (
                    f"{evidence_row['status_reason']} Frozen-source "
                    "wave-19 disposition evidence SHA-256 is "
                    f"{evidence_sha256}."
                ),
                "ticket_duplicate_semantics": (
                    "NOT_APPLICABLE_CLOSED_UNEXECUTABLE"
                ),
                "ticket_order_semantics": (
                    "NOT_APPLICABLE_CLOSED_UNEXECUTABLE"
                ),
                "unranked_reason": (
                    f"CLOSED_UNEXECUTABLE:{reason_code}"
                ),
            }
        )

    artifacts_raw = catalog.get("source_artifacts")
    if not isinstance(artifacts_raw, list):
        raise CatalogOverlayError("base source artifacts changed")
    cast(list[object], artifacts_raw).append(
        {
            "artifact_name": evidence_path.name,
            "artifact_sha256": evidence_sha256,
            "evidence_role": (
                "STATIC_DISPOSITION_WAVE19_NON_BIGLOTTO_AND_IMPORTED_"
                "PREDICTOR_HARNESS_REVIEW"
            ),
        }
    )
    catalog["status_counts"] = EXPECTED_OUTPUT_STATUS_COUNTS
    catalog["catalog_sha256"] = _catalog_hash(catalog)
    return cast(dict[str, object], catalog)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--base-catalog", type=Path, required=True)
    parser.add_argument("--evidence", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    catalog = apply_wave19_evidence(
        base_catalog_path=args.base_catalog,
        evidence_path=args.evidence,
    )
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_bytes(_canonical_bytes(catalog) + b"\n")


if __name__ == "__main__":
    main()
