#!/usr/bin/env python3
"""Build compact evidence for the wave-24 causal source-native batch."""
...
from lottolab.infrastructure.legacy_source_native_batch_import_wave24 import (
...
EXPECTED_INPUT_CANONICAL_SHA256 = (
    "9c24650b9f2b11d1b4cf83e5c8701ecd21af343c0e67ae2c6b0fff90f8511dc1"
)
...
EXPECTED_REPORT_FILE_SHA256 = (
    "ebfe9325883a4037db2c68b1166fd21618481f049646fd327d550341b2c9dbda"
)
...
EXPECTED_PROGRESS = {
    ...
    "owner_decision_required_count": 125,
    "reproduced_count": 54,
    "total_strategy_count": 221,
    "uncompleted_count": 125,
}
EXPECTED_REPORT_CHECKSUMS = {
    "SHA256SUMS": (
        "292b54b04f692f6844b7f807616a0d7944283f55be92e4cd573862d62601511d"
    ),
    "biglotto_execution_audit.csv": (
        "617f16086de4ff477b74546683d12afbd5a5e7db54528292f31b84310b7836ca"
    ),
    "biglotto_full_rankings.csv": (
        "2020ec4c8193937c51596a31c69f2f203d9b6f8c4cae5c7cfb0f3cae04f7a929"
    ),
    "biglotto_multi_ticket_backtest_report.json": (
        "ebfe9325883a4037db2c68b1166fd21618481f049646fd327d550341b2c9dbda"
    ),
    "biglotto_official_prize_distributions.csv": (
        "bcd86423693c2c1f32a48753b0d930e2aaed82337c35ac982be72277154560f4"
    ),
    "biglotto_strategy_universe.csv": (
        "a15a6349077bf905776e0f372bc827665c566e405e767a74a8de95899ec60da7"
    ),
    "biglotto_success_metrics.csv": (
        "c66d6c6f4a67e1ea8216e282addc1f2c15db8aeb356f981334483d1a7f6a1c88"
    ),
    "biglotto_top10.csv": (
        "35cb7dcaee6d509f6cb5458fe6c37bde9d2ba1322a8f4e4f11a9fd76395b5c29"
    ),
}
...
EXPECTED_CANDIDATE_K_DISTRIBUTIONS = {
    TWO_BET_FINAL_METHOD_ID: {12: 1, 13: 11, 14: 65, 15: 2071},
    THREE_BET_OPTIMIZER_METHOD_ID: {
        14: 3,
        15: 13,
        16: 19,
    ...
    DCB_METHOD_ID: {15: 3, 16: 3, 17: 8, 18: 2134},
    FOUR_BET_DCB_METHOD_ID: {18: 2134},
    ECP_METHOD_ID: {
        14: 3,
        15: 13,
        16: 19,
...
EXPECTED_MARKOV_ORDER_DISTRIBUTIONS = {
    TWO_BET_FINAL_METHOD_ID: {1: 49, 2: 100, 3: 1999},
    THREE_BET_OPTIMIZER_METHOD_ID: {1: 48, 2: 100, 3: 1999},
    "tools/test_asm.py": {1: 49, 2: 100, 3: 1999},
    DCB_METHOD_ID: {1: 49, 2: 100, 3: 1999},
    FOUR_BET_DCB_METHOD_ID: {1: 35, 2: 100, 3: 1999},
    ECP_METHOD_ID: {1: 48, 2: 100, 3: 1999},
}
EXPECTED_KILL_COUNT_DISTRIBUTIONS = {
    TWO_BET_FINAL_METHOD_ID: {0: 2148},
    THREE_BET_OPTIMIZER_METHOD_ID: {0: 28, 5: 2119},
    "tools/test_asm.py": {0: 29, 5: 2119},
    DCB_METHOD_ID: {0: 29, 5: 2119},
    FOUR_BET_DCB_METHOD_ID: {0: 15, 5: 2119},
    ECP_METHOD_ID: {0: 28, 5: 2119},
}


class EvidenceBuildError(ValueError):
    """Wave-24 evidence inputs violate the frozen contract."""
...
def _validate_catalog(path: Path) -> None:
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE24_METHODS:
        ...
        if (
            ...
            raise EvidenceBuildError(
                "wave-24 catalog identity changed"
            )
...
def _validate_input(
    ...
    for candidate in executions:
        ...
        if method_id not in SUPPORTED_SOURCE_NATIVE_WAVE24_METHODS:
        ...
        if (
            type(row.get("candidate_k")) is not int
            or row.get("candidate_k") != len(candidate_pool)
            or row.get("combination_count")
            != expected_combination_count
            or row.get("native_ticket_count") != expected_ticket_count
            or native.get("candidate_k") is not None
            or native.get("candidate_pool_size") != len(candidate_pool)
            ...
            != [
                list(item)
                for item in FROZEN_SUPPORT_ARTIFACTS_BY_SOURCE_NATIVE_WAVE24_METHOD[
                    typed_method_id
                ]
            ]
        ...
        candidates_by_method[typed_method_id][len(candidate_pool)] += 1
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE24_METHODS:
        if (
            ...
            or duplicates_by_method[method_id]
            != Counter({0: EXPECTED_STATUS_BY_METHOD[method_id]["OK"]})
        ...
        expected_calls = 50 if method_id == ECP_METHOD_ID else 1
...
def build_evidence(
    *,
    ...
    if (
        ...
        or parity.get("case_count") != 198
        or len(parity_cases) != 198
        or sum(
            1
