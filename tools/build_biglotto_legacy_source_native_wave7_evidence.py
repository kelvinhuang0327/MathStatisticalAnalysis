"""Build compact evidence for the seventh source-native BIG_LOTTO wave."""
...
from lottolab.infrastructure.legacy_source_native_batch_import_wave7 import (
    MATERIALIZATION_SCHEMA_VERSION,
)
from verify_biglotto_legacy_source_native_wave7_parity import (
...
EXPECTED_REPORT_PROGRESS = {
    ...
    "owner_decision_required_count": 169,
    "reproduced_count": 27,
    "total_strategy_count": 221,
    "uncompleted_count": 169,
}


class EvidenceBuildError(ValueError):
    """The wave-7 evidence inputs violate the frozen contract."""
...
def _validate_parity(
    *,
    ...
    if (
        ...
        or parity.get("database_sha256") != dataset_sha256
        or parity.get("port_protocol") != SOURCE_NATIVE_WAVE7_PROTOCOL
    ...
    if not isinstance(cases_raw, list):
        raise EvidenceBuildError("frozen-source parity cases changed")
    cases = cast(list[object], cases_raw)
    if len(cases) != 15:
        raise EvidenceBuildError("frozen-source parity cases changed")
    method_counts: Counter[str] = Counter()
    for candidate in cases:
        if not isinstance(candidate, dict):
            raise EvidenceBuildError("frozen-source parity case is invalid")
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE7_METHODS
            or row.get("history_draw_count") not in (10, 150, 2148)
            or type(row.get("native_ticket_count")) is not int
            or type(row.get("seed_digest")) is not str
            or len(cast(str, row["seed_digest"])) != 64
    ...
    if method_counts != Counter(
        ...
        raise EvidenceBuildError(
            "frozen-source parity method coverage changed"
        )
    return {
...
def _validate_report_contract(
    *,
    ...
    expected_lengths = {
        "execution_audit": 10745,
        "metrics": 640,
        "official_prize_distributions": 80,
        "rankings": 28288,
        "top_10": 640,
        "universe": 221,
    }
...
def _validate_provenance(provenance: dict[str, Any]) -> None:
    expected_combination_members = {
        ...
        for method_id, members in (
            SOURCE_COMBINATION_MEMBERS_BY_SOURCE_NATIVE_WAVE7_METHOD.items()
        )
    }
    expected_candidate_counts = {
        method_id: list(counts)
        for method_id, counts in (
            SOURCE_CANDIDATE_TICKET_COUNTS_BY_SOURCE_NATIVE_WAVE7_METHOD.items()
        )
    }
    if (
...
def build_evidence(
    *,
    ...
    for candidate in cast(list[object], universe_raw):
        ...
        row = cast(dict[str, Any], candidate)
        method_id = row.get("legacy_method_id")
        if method_id in SUPPORTED_SOURCE_NATIVE_WAVE7_METHODS:
    ...
    if set(catalog_by_method) != set(
        ...
        raise EvidenceBuildError("wave-7 methods leave the 221 universe")
    method_by_strategy = {
    ...
    closed_counts: dict[str, Counter[str]] = defaultdict(Counter)
    ...
    for candidate in cast(list[object], executions_raw):
        ...
        if method_id is None:
            raise EvidenceBuildError(
                "execution strategy is outside wave 7"
            )
        ...
        if status == "OK":
            ...
            closed_counts[method_id][status] += 1
            ...
            if row.get("reason_code") not in {
                ...
                raise EvidenceBuildError(
                    "unexpected frozen-source execution error"
                )
            closed_counts[method_id][status] += 1
        else:
            raise EvidenceBuildError(
        ...
        strategies.append(
            {
                ...
                "closed_status_counts": dict(
                    sorted(closed_counts[method_id].items())
                ),
                "combination_count": (
                    SOURCE_COMBINATION_COUNT_BY_SOURCE_NATIVE_WAVE7_METHOD[
                        method_id
    ...
    return {
        ...
        "user_seed": DEFAULT_SOURCE_NATIVE_WAVE7_USER_SEED,
    }
