#!/usr/bin/env python3
"""Build checked evidence for the eighth BIG_LOTTO source-native batch."""
...
from verify_biglotto_legacy_source_native_wave8_parity import (
...
EXPECTED_STATUS_COUNTS_BY_METHOD = {
    "tools/verify_gemini_phase2_claim.py": {
    ...
    "tools/research_cluster_enhancements.py": {
        "CLOSED_EXECUTION_ERROR": 1880,
        "CLOSED_INSUFFICIENT_HISTORY": 100,
        "OK": 169,
    },
    "tools/optimize_3rd_bet.py": {
        "CLOSED_INSUFFICIENT_HISTORY": 1,
        "OK": 2148,
    },
}
EXPECTED_NATIVE_COUNT_VALUES = {
    "tools/verify_gemini_phase2_claim.py": [7],
    "tools/dynamic_frequency_predictor.py": [1],
    "tools/research_cluster_enhancements.py": [14, 15],
    "tools/optimize_3rd_bet.py": [1],
}
EXPECTED_REPORT_PROGRESS = {
    "backtested_count": 31,
    "closed_count": 21,
    "duplicate_alias_count": 4,
    "owner_decision_required_count": 165,
    "reproduced_count": 31,
    "total_strategy_count": 221,
    "uncompleted_count": 165,
}


class EvidenceBuildError(ValueError):
    """Wave-8 evidence inputs violate the frozen contract."""
...
def _validate_parity(
    ...
    if (
        ...
        or parity.get("port_protocol") != SOURCE_NATIVE_WAVE8_PROTOCOL
        ...
        or len(cast(list[object], cases_raw)) != 12
    ):
    ...
    for candidate in cast(list[object], cases_raw):
        ...
        if (
            method_id not in SUPPORTED_SOURCE_NATIVE_WAVE8_METHODS
...
def _strategy_rows(
    ...
    candidate_k_values: dict[str, set[int]] = defaultdict(set)
    candidate_combination_values: dict[str, set[int]] = defaultdict(set)
    ...
    for candidate in cast(list[object], executions_raw):
        ...
        if status == "OK":
            ...
            if method_id not in SUPPORTED_SOURCE_NATIVE_WAVE8_METHODS:
            ...
            if (
                ...
                raise EvidenceBuildError("native count changed")
            native_counts[method].add(native_count)
            duplicate_counts[method].add(duplicate_count)
            if type(row.get("candidate_k")) is int:
                candidate_k_values[method].add(
                    cast(int, row["candidate_k"])
                )
            if type(row.get("candidate_combination_count")) is int:
                candidate_combination_values[method].add(
                    cast(int, row["candidate_combination_count"])
                )
            continue
        method = next(
            (
    ...
    for method_id in SUPPORTED_SOURCE_NATIVE_WAVE8_METHODS:
        ...
        rows.append(
            {
                "candidate_combination_count_values": sorted(
                    candidate_combination_values[method_id]
                ),
                "candidate_k": (
                    CANDIDATE_K_BY_SOURCE_NATIVE_WAVE8_METHOD[
                        method_id
...
def build_evidence(
    *,
    ...
    if (
        ...
        or len(cast(list[object], executions)) != 8596
    ...
    expected_lengths = {
        "execution_audit": 8596,
        "metrics": 512,
        "official_prize_distributions": 64,
        "rankings": 28288,
        "top_10": 512,
        "universe": 221,
    }
    ...
    return {
        ...
        "source_native_protocol": SOURCE_NATIVE_WAVE8_PROTOCOL,
