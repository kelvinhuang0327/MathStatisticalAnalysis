"""Render the concise Phase-7 P638 Zone-1 replication report from the locked result."""
...
OUTPUT_PATH = Path(
    "docs/research/matrix-native-results/"
    "constructor-frontier-next-generation-p638-zone1-v1-report.md"
)
...
def main() -> None:
    ...
    gate = result["p638_replication_gate"]
    rows: list[str] = []
    float_rows: list[str] = []
    geom_rows: list[str] = []
    ...
    report = f"""# STRATEGY_MATRIX_PHASE7_P638_ZONE1_NEXT_GEN_CONSTRUCTOR_REPLICATION_V1 — result
...
```text
P638_REPLICATION_GATE: {gate_status}
P638_REPLICATION_STATUS: {result["p638_replication_status"]}
CROSS_LOTTERY_STATUS: {result["cross_lottery_status"]}
```
