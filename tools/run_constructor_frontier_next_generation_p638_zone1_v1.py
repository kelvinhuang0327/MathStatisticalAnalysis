"""Execute the locked P638 Zone-1 next-generation constructor replication.

Re-verifies the LCJ-1 lock hash and native (38, 6) mapping before
generating any native candidate portfolio or inspecting `Q_E`. B649 and
T539 are not rerun. Arm-C is not manufactured. Zone-2 is not executed.
"""
...
AUTHORITY_STOP = "STOP_PHASE7_P638_AUTHORITY_UNRESOLVED"
...
def constructor_function_has_foreign_hardcodes(source: str) -> bool:
    function_body = source.split("def greedy_minmax_then_sum_overlap_portfolio", 1)[1]
    return "49" in function_body or "39" in function_body


def assert_p638_native_mapping(locked: dict[str, Any]) -> None:
    if POWER_LOTTO_RULE_CONTRACT.main_number_max != 38:
        raise ValueError(NATIVE_MAPPING_STOP)
    if POWER_LOTTO_RULE_CONTRACT.main_number_count != 6:
        raise ValueError(NATIVE_MAPPING_STOP)
    if locked["lottery_type"] != "POWER_LOTTO":
        raise ValueError(NATIVE_MAPPING_STOP)
    if locked["pool_size"] != POWER_LOTTO_RULE_CONTRACT.main_number_max:
        raise ValueError(NATIVE_MAPPING_STOP)
    if locked["draw_size"] != POWER_LOTTO_RULE_CONTRACT.main_number_count:
        raise ValueError(NATIVE_MAPPING_STOP)
    if locked["p638_zone"] != "zone1":
        raise ValueError(NATIVE_MAPPING_STOP)
    ...
    if constructor_function_has_foreign_hardcodes(source):
        raise ValueError(NATIVE_MAPPING_STOP)
...
def verify_lock_inputs(locked: dict[str, Any]) -> None:
    ...
    if _git_blob(locked["sealed_sidon_result_path"]) != locked["sealed_sidon_result_blob_sha1"]:
        raise ValueError("sealed P638 Sidon artifact drifted from the lock")
    if _git_blob(locked["sealed_arm_b_result_path"]) != locked["sealed_arm_b_result_blob_sha1"]:
        raise ValueError("sealed P638 Arm-B artifact drifted from the lock")
    ...
    if _git_blob(locked["t539_phase7_result_path"]) != locked["t539_phase7_result_blob_sha1"]:
        raise ValueError("sealed T539 Phase-7 artifact drifted from the lock")
...
def verify_b649_authority(locked: dict[str, Any]) -> None:
    ...
    if payload["execution_classification"] != "B649_NEXT_GEN_CONSTRUCTOR_ADVANCE":
        raise ValueError(AUTHORITY_STOP)
...
def verify_t539_authority(locked: dict[str, Any]) -> None:
    payload = json.loads(Path(locked["t539_phase7_result_path"]).read_text(encoding="utf-8"))
    if payload["constructor_id"] != locked["t539_phase7_constructor_id"]:
        raise ValueError(AUTHORITY_STOP)
    if payload["t539_replication_status"] != locked["t539_phase7_status"]:
        raise ValueError(AUTHORITY_STOP)
    if payload["p638_replication_eligible"] is not True:
        raise ValueError(AUTHORITY_STOP)
    locked_hash = payload["canonical_input"]["locked_preregistration_sha256"]
    if locked_hash != locked["t539_phase7_preregistration_hash_sha256"]:
        raise ValueError(AUTHORITY_STOP)
...
def evaluate_p638_replication_gate(
    ladder: list[int],
    ...
    return {
        "classification": (
            "P638_NEXT_GEN_CONSTRUCTOR_REPLICATION_SUPPORTED"
            if passed
            else "DO_NOT_ADVANCE_THIS_EXACT_P638_REPLICATION"
        ),
        "clauses": clauses,
        "cross_lottery_status": (
...
def verify_sealed_p638_comparators(locked: dict[str, Any]) -> None:
    ...
    if sidon.get("zone") != "zone1" or arm_b.get("zone") != "zone1":
        raise ValueError(AUTHORITY_STOP)
...
def run(locked: dict[str, Any]) -> dict[str, Any]:
    assert_p638_native_mapping(locked)
    verify_lock_inputs(locked)
    verify_b649_authority(locked)
    verify_t539_authority(locked)
    verify_sealed_p638_comparators(locked)
    ...
    gate = evaluate_p638_replication_gate(
    ...
    return {
        ...
        "cross_lottery_status": gate["cross_lottery_status"],
        ...
        "monte_carlo": False,
        "p638_replication_gate": gate,
        "p638_replication_status": gate["classification"],
        "p638_zone": "zone1",
        "p638_zone2": "NOT_RUN",
...
def main() -> None:
    ...
    print(f"cross_lottery_status={result['cross_lottery_status']}")
