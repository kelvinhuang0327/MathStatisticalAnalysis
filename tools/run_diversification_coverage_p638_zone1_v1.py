"""Execute the locked DIVERSIFICATION_COVERAGE_P638_ZONE1_V1 experiment.
...
complete C(38,6) Zone-1 winning-space enumeration and the already-verified
`exact_coverage_baseline` closed-form random portfolio baseline, reused
unmodified from the B649/T539 cells. Mirrors
`run_diversification_coverage_t539_v1.py` exactly, generalized to draw_size=6
so the secondary-event ladder includes M6 (T539's draw_size=5 has no M6).
Zone-2 (1-of-8) is never read, assigned, or evaluated here.
"""
...
from lottolab.research.cyclic_sidon_shift_p638 import sidon_shift_portfolio
...
def run(locked: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "zone": locked["zone"],
        "replicates": [
            "DIVERSIFICATION_COVERAGE_B649_V1",
            "DIVERSIFICATION_COVERAGE_T539_V1",
        ],
