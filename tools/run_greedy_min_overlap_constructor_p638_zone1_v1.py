"""Execute the locked GREEDY_MIN_OVERLAP_CONSTRUCTOR_P638_ZONE1_V1 experiment.

Reads locked parameters from
`docs/research/matrix-native-results/greedy-min-overlap-constructor-p638-zone1-v1-preregistration-hash.json`
...
uses). Computes, at real P638 Zone-1 scale (`pool_size=38, draw_size=6`,
for the first time ever):

- arm A (`CYCLIC_SIDON_SHIFT_P638_ZONE1_V1`, immutable, unmodified) and arm
  B (`GREEDY_MIN_OVERLAP_CONSTRUCTOR_P638_ZONE1_V1`, first real-scale
  invocation) via the same single-pass earliest-index enumeration method
  the sealed `DIVERSIFICATION_COVERAGE_P638_ZONE1_V1` cell and
  ...
  finding (`strategy-matrix-phase5-p638-non-sidon-low-overlap-native-design-r1.md`
...
the already-sealed `DIVERSIFICATION_COVERAGE_P638_ZONE1_V1` cell's own
...
counterpart here -- three arms only, per the Owner packet. B649 and T539
are not rerun by this task; their own sealed results are cited as given
for the Q3 direction-consistency check (design doc S10).
"""
...
def run(locked: dict[str, Any]) -> dict[str, Any]:
    ...
    # Arm B: NON_SIDON_LOW_OVERLAP, real P638 Zone-1 scale for the first time (nested prefix)
    t0 = time.perf_counter()
    greedy_full = greedy_min_overlap_portfolio_p638_zone1(max_k)
    ...
    # already-sealed DIVERSIFICATION_COVERAGE_P638_ZONE1_V1 cell's own q_sidon values.
    ...
    q1_classification = _classify_by_signs(
        ...
        "P638_ARM_B_DOES_NOT_OUTPERFORM_RANDOM",
        "P638_ARM_B_MIXED_BY_EXPOSURE",
    )
    q2_classification = _classify_by_signs(
        [delta_sidon[primary_m][k] for k in ladder_gt1],
        "P638_ARM_B_EXCEEDS_SIDON_GAIN",
        "P638_ARM_B_DOES_NOT_EXCEED_SIDON_GAIN",
        "P638_ARM_B_MIXED_VS_SIDON",
    )
    # B649 (DIVERSIFICATION_CONSTRUCTOR_FRONTIER_B649_V1, sealed) and T539
    # (GREEDY_MIN_OVERLAP_CONSTRUCTOR_T539_V1, sealed T539_REPLICATION_SUPPORTED)
    # both already found arm B's DELTA_SIDON(k) > 0 at every tested k>1 --
    # cited as given, not rerun here (preregistration S6 / design doc S10).
    q3_classification = (
        "CONSISTENT_WITH_B649_AND_T539"
        if q2_classification == "P638_ARM_B_EXCEEDS_SIDON_GAIN"
        else "DIRECTION_INCONSISTENT_WITH_B649_AND_T539"
    )

    p638_replication_supported = (
        q1_classification == "P638_ARM_B_OUTPERFORMS_RANDOM"
        and q2_classification == "P638_ARM_B_EXCEEDS_SIDON_GAIN"
        and q3_classification == "CONSISTENT_WITH_B649_AND_T539"
    )
    p638_replication_status = (
        "P638_REPLICATION_SUPPORTED" if p638_replication_supported else "NOT_SUPPORTED"
    )
    # P638 Zone-1 is the last of the three native lottery structures this
    # repository currently supports for this arm-B translation chain
    # (design doc S10 replication-closure note) -- a PASS here closes the
    # chain rather than naming a next replication target.
    non_sidon_low_overlap_cross_lottery_status = (
        "SUPPORTED_IN_3_NATIVE_LOTTERY_STRUCTURES"
        if p638_replication_supported
        else "NOT_SUPPORTED"
    )
    ...
    return {
        ...
        "p638_replication_status": p638_replication_status,
        "non_sidon_low_overlap_cross_lottery_status": non_sidon_low_overlap_cross_lottery_status,
...
def main() -> None:
    ...
    print(f"p638_replication_status: {result['p638_replication_status']}")
    print(
        "non_sidon_low_overlap_cross_lottery_status: "
        f"{result['non_sidon_low_overlap_cross_lottery_status']}"
    )
