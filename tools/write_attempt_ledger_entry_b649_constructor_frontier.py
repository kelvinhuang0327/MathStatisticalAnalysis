"""Write the single attempt-ledger record for the B649 constructor-frontier experiment.

Mirrors `write_attempt_ledger_entry_p638.py` / `write_attempt_ledger_entry_t539.py`
exactly. `result_hash_sha256` uses plain JSON canonicalization (sorted keys,
SHA-256), not LCJ-1: this result blob legitimately contains floats (reported
alongside exact fraction strings), which LCJ-1 forbids by design for the
strict, integer-only `locked_parameters` document.
"""
...
OUTPUT_PATH = Path(
    ...
    "diversification-constructor-frontier-b649-v1-attempt-ledger.json"
)
...
def main() -> None:
    ...
    entry: dict[str, Any] = {
        "attempt_id": "DIVERSIFICATION_CONSTRUCTOR_FRONTIER_B649_V1__ATTEMPT_1",
        "sequence": 1,
