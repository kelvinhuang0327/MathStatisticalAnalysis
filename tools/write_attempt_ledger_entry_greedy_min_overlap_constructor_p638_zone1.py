"""Write the single attempt-ledger record for the P638 Zone-1 greedy-constructor experiment.

Mirrors `write_attempt_ledger_entry_greedy_min_overlap_constructor_t539.py` /
`write_attempt_ledger_entry_p638.py` exactly. `result_hash_sha256` uses plain
JSON canonicalization (sorted keys, SHA-256), not LCJ-1: this result blob
legitimately contains floats (reported alongside exact fraction strings),
which LCJ-1 forbids by design for the strict, integer-only
...
OUTPUT_PATH = Path(
    ...
    "greedy-min-overlap-constructor-p638-zone1-v1-attempt-ledger.json"
)
...
def main() -> None:
    ...
    entry: dict[str, Any] = {
        "attempt_id": "GREEDY_MIN_OVERLAP_CONSTRUCTOR_P638_ZONE1_V1__ATTEMPT_1",
        "sequence": 1,
