"""Write the single attempt-ledger record for the T539 greedy-constructor experiment.

Mirrors `write_attempt_ledger_entry_t539.py` /
`write_attempt_ledger_entry_b649_constructor_frontier.py` exactly.
`result_hash_sha256` uses plain JSON canonicalization (sorted keys,
SHA-256), not LCJ-1: this result blob legitimately contains floats
(reported alongside exact fraction strings), which LCJ-1 forbids by
design for the strict, integer-only `locked_parameters` document.
"""
...
def main() -> None:
    ...
    entry: dict[str, Any] = {
        "attempt_id": "GREEDY_MIN_OVERLAP_CONSTRUCTOR_T539_V1__ATTEMPT_1",
        "sequence": 1,
